<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Service List</h5>
        <div class="bg-toolbar my-2 p-1">

            <div class="d-flex justify-content-end align-items-center">

                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/services" v-if="isCreatePermitted">Add Service</router-link>
                </div>
...
<!--                        @sort-changed="sortByAscDesc"-->
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #head()="data">
                                <div class="d-flex mx-1 custom-input-header">
                                    <div class="mx-1 btn-group">
                                        ...
                                        <input @input="debounce" @keyup.enter="filterRecords(1)" class="form-control-sm border rounded-0 custom-input-width" placeholder="Search here..." type="text" v-model="queryFields[data.field.parent].find(item => item.column == data.column).value"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #cell(image)="data">
                                <img :src="`images/service/${data.value}`" class="ml-1" height="38" v-if="data.value !=''" width="58"/>
                            </template>

                            <template #cell(price)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.price ? data.item.price.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(override_selling_price)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.override_selling_price ? data.item.override_selling_price.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(freight)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.freight ? data.item.freight.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(product_cost)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.product_cost ? data.item.product_cost.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'services/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                // this.$store.getters.sortByAlphaNumeric({'module': 'service', 'fieldName': 'product_number'});
                return this.$store.getters.getServiceTableData;
            },
            perPageRecordList(){
        ...
        methods: {
                ...
                if (confirm("Do you really want to delete this record?")) {
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data) {
                            ...
                            /*this.$store.dispatch('getModuleInfo', {module:'composite'});
            ...
            sortByAscDesc: function () {
                ...
                let tableNames = {
                    'Category': {'columnTableName': 'idInParentTable', 'foreignKey': 'category_id', 'childClassName' : '\\ServiceCategory'},
                    'Service': {'columnTableName': 'service', 'childClassName' : ''}
                };
