<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Service</h5>
            <div class="bg-toolbar my-2 p-1">
                <Toolbar :id="formFields.id" :saveDisabled="saveDisabled"></Toolbar>
            </div>
        </div>
        <div class="col-md-12 custom-margin pr-1 my-2" id="service">
            <form class="div-shadow bg-white rounded" id="monument-form">
                ...
                <template v-else>
                            ...
                            <b-dropdown :disabled="hideInModal" class="input-xl my-auto border rounded ml-0 cutsom-width-dropdown" no-caret size="sm" toggle-class=""
                                ...
                                <template #button-content>
                                    <div class="d-flex product-color">
                                        <small>{{customizeLabel(serviceCategoryTitle)}}</small>
                                ...
                                <b-dropdown-group class="overflow-auto scroll px-1">
                                    ...
                                    <template v-if="serviceCategory.length > 0">
                                        <template v-for="(obj, key) of serviceCategory">
                    ...
                    <div class="product-row-6">
                        ...
                        <div v-if="!hideInModal">
                            <button :disabled="qtySaleHistory" class="btn-info mx-4 custom-infobtn-toolbar custom-infobtn-text" type="button"
                    ...
                    <div class="product-row-2">
                        <div>
                            ...
                            <b-form-input :disabled="hideInModal || hideinAuth" class="serviceTypeMarginLeft" name="service_type"
                                          size="sm" type="text" v-model="formFields.type"></b-form-input>
                        </div>
                        <div>
                            <label class="ml-4">Color</label>
                            <b-form-input :disabled="hideInModal || hideinAuth" class="ml-1" name="service_color"
                                          size="sm" type="text" v-model="formFields.color"></b-form-input>
                        </div>
                        <div>
                            <label class="ml-4">Shape</label>
                            <b-form-input :disabled="hideInModal || hideinAuth" class="ml-1" name="service_shape"
                                          size="sm" type="text" v-model="formFields.shape"></b-form-input>
                        </div>
                        <div>
                            <label class="ml-4">Weight</label>
                            <b-form-input :disabled="hideInModal || hideinAuth" class="ml-1" min="0" name="service_weight"
                        ...
                        <div class="col-md-3 flex-wrap m-1">
                            <template v-if="this.formFields.files">
                                <template v-for="(file,index) of this.formFields.files">
                                    <div class="flex-column border mx-1 mb-2 py-1 justify-content-center file-container">
                                        <i :title="`${file.name}`" class="fa fa-file fa-2x m-auto" @click="openPreviewNewPage(imagePath+file.file)"
                                           v-b-tooltip.hover style="cursor: pointer;"></i>
        ...
        <b-modal hide-footer hide-header hide-header-close id="posted-product-inventory">
            <InventoryHistory :formRecord="serviceInventory" :id="formFields.id" :inventoryName="'service_inventory'" :module="this.axiosParams.module"></InventoryHistory>
        </b-modal>
        <SaleHistory :invoice="'service_invoice'" :module="this.axiosParams.module" :recordID="formFields.id"></SaleHistory>
    </div>
</template>
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                axiosParams: {module: 'service'},
                axiosParamsComposite: {module: 'composite'},
                axiosParamsCategory: {module: 'serviceCategory'},
                searchCategory: "",
                alertMessage: '',
                hideAlert: true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Service`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_SERVICE, this.storeService);
            eventBus.$on(event.FIRST_SERVICE_RECORD, this.first);
            eventBus.$on(event.LAST_SERVICE_RECORD, this.last);
            eventBus.$on(event.NEXT_SERVICE_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_SERVICE_RECORD, this.prev);
            eventBus.$on(event.GET_SERVICE, this.getDropDownById);
            eventBus.$on(event.ADD_SERVICE, this.clearForm);
            eventBus.$on(event.DELETE_SERVICE, this.deleteRecord);
        ...
        destroyed() {
            eventBus.$off(event.SAVE_SERVICE, this.storeService);
            eventBus.$off(event.FIRST_SERVICE_RECORD, this.first);
            eventBus.$off(event.LAST_SERVICE_RECORD, this.last);
            eventBus.$off(event.NEXT_SERVICE_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_SERVICE_RECORD, this.prev);
            eventBus.$off(event.GET_SERVICE, this.getDropDownById);
            eventBus.$off(event.ADD_SERVICE, this.clearForm);
            eventBus.$off(event.DELETE_SERVICE, this.deleteRecord);
        ...
        computed: {
            ...
            serviceCategoryTitle() {
                let opt = "Select";
                if (this.$store.state[this.axiosParams.module].serviceInfo.length > 0 && this.formFields.category_id != 0 && this.formFields.category_id != null) {
                    opt = this.$store.state[this.axiosParams.module].serviceInfo.filter(obj => obj.id == this.formFields.category_id);
            ...
            serviceCategory() {
                let options = [];
                if (this.$store.state[this.axiosParams.module].serviceInfo) {
                    this.$store.getters.sortByAlphaNumeric({'fieldName': 'title', 'recordList': this.$store.state[this.axiosParams.module].serviceInfo});
                    for (let opt of this.$store.state[this.axiosParams.module].serviceInfo) {
            ...
            serviceInventory() {
                if (!this.formFields.service_inventory) this.formFields.service_inventory = [];
                if (!this.hideInModal) {
                    if (this.formFields.service_inventory?.length && this.formFields.id) {
                        this.$store.getters.getProductInventory({
                            data: this.formFields.service_inventory,
                            module: this.axiosParams.module,
                            id: this.formFields.id,
                            childModule: 'service_inventory'
                        });
                    }
                }
                return this.formFields.service_inventory;
            },
            uploadDocuments() {
        ...
        mounted() {
            ...
            this.getService(this.formFields.id);
            this.getColumns();
            this.getRecords();
        ...
        methods: {
            ...
            /** CRUD */
            getDropDownById: async function (id) {
            ...
            storeService: function () {
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo) {
                        ...
                        this.makeToast({title: 'Service ' + status, message: this.alertMessage, variant: 'success'});
                        this.image = null;
                        this.$refs['inputFile'].reset();
                        this.formFields = response.data;
                        this.imageData = this.formFields.image ? this.imagePath + this.formFields.image : this.imageDefault;
            ...
            deleteDropDown: function (id) {
                if (confirm('Do you really want to delete this record ?')) {
                    ...
                    deleteRecord({id: id, module: moduleName}, (response) => {
                        let list = this.$store.state[this.axiosParams.module]['serviceInfo'];
            ...
            saveDetail: function (detail) {
                ...
                saveRecord(params, formData, (response) => {
                    if (response.data.errorInfo) {
                        ...
                        this.$store.state[this.axiosParams.module]['serviceInfo'].splice(this.$store.state[this.axiosParams.module]['serviceInfo'].length + 1, 0, response.data)
                    }
                }, (error) => {
                    this.makeToast({title: 'Error', message: error, variant: 'danger', duration: 3000});
                })
            ...
            clearForm: function () {
                ...
                if (!window.detectChanges) {
                    ...
                    this.formFields = {
                        ...
                        // store_id: this.storeID,
                        stock_info: 'stock', weight: null, type: null, color: null, category_id: null
                    };
                    this.image = null;
                }
            },
            ...
            addOrUpdateQtyInventory: function (qty) {
                ...
                if (qty) {
                    if (!this.formFields.service_inventory) this.formFields.service_inventory = [];
                    this.formFields.service_inventory.push({
...
<style scoped>
    ...
    .product-row-1, .product-row-2, .product-row-3, .product-row-5, .product-row-4, .product-row-6 {
        display: flex;
        margin: 0.5rem 0;
        padding-left: 1rem;
    }

    .product-row-1 div, .product-row-2 div, .product-row-3 div, .product-row-5 div, .product-row-4 div, .product-row-6 div {
        display: flex;
    }

    .product-row-1 div label, .product-row-2 div label, .product-row-3 div label, .product-row-5 div label, .product-row-4 div label, .product-row-6 div label {
        margin: auto 0.5rem;
