<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Daily Installation Load Check List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <router-link class="btn custom-infobtn-list custom-infobtn-list-text ml-4" to="/installation-load" v-if="isCreatePermitted">Add New</router-link>
                <div class="d-flex m-auto">
                    <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/installation-list">Back To Installation</router-link>
                </div>
                <div class="d-flex">
                </div>
                    ...
                    <div class="installation-body p-3">
                        <div class="d-flex">
                            <b-dropdown
                                ...
                                <b-dropdown-form class="overflow-auto">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #head(final_date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <b-form-datepicker @input="filterRecords(1)" class="form-control-sm rounded-0 border input-date" close-button locale="en" placeholder=""
                                                           reset-button
                                                           size="sm"
                                                           today-button
                            ...
                            <template #head(date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        <select @change="filterRecords(1)" class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).operator">
                                            <option selected title="Equals To" v-b-tooltip.hover value="=">=</option>
                                            <option title="Not Equals to" v-b-tooltip.hover value="!="><></option>
                                            <option title="Greater Than" v-b-tooltip.hover value=">">></option>
                                            <option title="Less Than" v-b-tooltip.hover value="<"><</option>
                                            <option title="Greater & Equals to" v-b-tooltip.hover value=">=">>=</option>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'installation-load/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'InstallationDailyLoad'" :recordID="viewRecord" :title="'Installation Daily Load'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getInstallerTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Installer': {'columnTableName': 'installer', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'installer';
