<template>
    <div>
        <div class="non-print-elm" v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading" style="margin-bottom: 0rem !important;">Installation Production - {{ formFields.id ? 'Edit' : 'Add'}}</h5>
        ...
        <PDF :title="'printInstallationProduction'">
            <div slot="header">
                <div class="d-flex justify-content-center">
                    <h3>INSTALLATION PRODUCTION FORM</h3>
                </div>
            </div>
            <div slot="body">
...
<script>
    ...
    export default {
        ...
        created() {
            eventBus.$on(event.SAVE_INSTALLATION_PRODUCTION, this.save);
            eventBus.$on(event.DELETE_INSTALLATION_PRODUCTION, this.deleteRecord);
            eventBus.$on(event.FIRST_INSTALLATION_PRODUCTION_RECORD, this.first);
            eventBus.$on(event.LAST_INSTALLATION_PRODUCTION_RECORD, this.last);
            eventBus.$on(event.NEXT_INSTALLATION_PRODUCTION_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_INSTALLATION_PRODUCTION_RECORD, this.prev);
            eventBus.$on(event.GET_INSTALLATION_PRODUCTION, this.getRecord);
            eventBus.$on(event.ADD_INSTALLATION_PRODUCTION, this.clearForm);
        },
        destroyed() {
            eventBus.$off(event.SAVE_INSTALLATION_PRODUCTION);
            eventBus.$off(event.DELETE_INSTALLATION_PRODUCTION);
            eventBus.$off(event.FIRST_INSTALLATION_PRODUCTION_RECORD,);
            eventBus.$off(event.LAST_INSTALLATION_PRODUCTION_RECORD);
            eventBus.$off(event.NEXT_INSTALLATION_PRODUCTION_RECORD);
            eventBus.$off(event.PREVIOUS_INSTALLATION_PRODUCTION_RECORD);
            eventBus.$off(event.GET_INSTALLATION_PRODUCTION);
            eventBus.$off(event.ADD_INSTALLATION_PRODUCTION);
        },
        beforeCreate() {
        ...
        methods: {
            ...
            getRecords() {
                ...
                fetchRecords({module: 'installationProduction/order'}, (response) => {
