<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Daily Truck Load List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <router-link class="btn custom-infobtn-list custom-infobtn-list-text ml-4" to="/truck-load" v-if="isCreatePermitted">Add New</router-link>
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'truck-load/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'TruckLoadList'" :recordID="viewRecord" :title="'Truck Load List'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getLoaderTableData;
            },
            isBusy() {
            ...
            filterFields() {
                ...
                for (const [key, value] of Object.entries(this.$store.state[this.axiosParams.module].columnList)) {
                    ...
                    for (let val of value) {
                        ...
                        if (val == 'truck_name_info')
                            label = 'Truck Name/Info'
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Loader': {'columnTableName': 'loader', 'childClassName': ''}
                };
