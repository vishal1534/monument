<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Weekly Schedule List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <router-link class="btn custom-infobtn-list custom-infobtn-list-text ml-4" to="/weekly-schedule" v-if="isCreatePermitted">Add New</router-link>
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'weekly-schedule/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'weeklySchedule'" :recordID="viewRecord" :title="'Weekly Schedule'"></Modal>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                sortByColumn: 'id',
                sortByDesc: false,
                columnTableName: 'weeklySchedule',
                foreignKey: '',
                childClassName: '',
                childTableName: '',
                sortDirection: 'desc',
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getWeeklyScheduleTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Weekly Schedule': {'columnTableName': 'weeklySchedule', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'weeklySchedule';
