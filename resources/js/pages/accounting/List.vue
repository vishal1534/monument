<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Accounting List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <!--<div class="d-flex m-auto">
                    <router-link v-if="isCreatePermitted" class="btn custom-infobtn-list custom-infobtn-list-text" to="/accountings">Add Accounting</router-link>
                </div>-->
                <div class="d-flex">
                </div>
...
<!--                            <label class="ml-auto custom-record-showing">Showing {{(currentPage*perPage) - (perPage-1)}} to {{rows < 10 || currentPage*perPage > rows ? rows : currentPage*perPage}} of {{rows}} records</label>-->
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                 ...
                                 bordered class="my-table bootstrap-vue-table a" hover id="data-table"
                            ...
                            <template #head()="data">
                                <div class="d-flex mx-1 custom-input-header">
                                    <div class="mx-1 btn-group">
                                        ...
                                        <input :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="debounce" class="form-control-sm border rounded-0 custom-input-width" placeholder="Search here..."
                                               type="text"
...
<!--                                <span class="custom-table-body ml-1"> {{ (data.index + 1) + ((currentPage-1) * perPage)}}</span>-->
                            ...
                            <template #cell(invoice_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order?.invoice?.invoice_total ? convertDecimal(Number(data.item.order?.invoice?.invoice_total)) : '0.00' }}</span>
                            </template>
                            <template #cell(sub_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order?.sub_total ? convertDecimal(Number(data.item.order?.sub_total)) : '0.00' }}</span>
                            </template>
                            <template #cell(tax_rate)="data">
                                <span class="custom-table-body ml-1">% {{ data.item.order?.tax_rate ? convertDecimal(Number(data.item.order?.tax_rate)) : '0.00' }}</span>
                            </template>
                            <template #cell(discount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order?.discount ? convertDecimal(Number(data.item.order?.discount)) : '0.00' }}</span>
                            </template>
                            <template #cell(tax_amount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order?.tax_amount ? convertDecimal(Number(data.item.order?.tax_amount)) : '0.00' }}</span>
                            </template>
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'accountings/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                        ...
                        <Modal :module="'Accounting'" :recordID="viewRecord" :title="'Accounting'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                //return this.$store.state[this.axiosParams.module].recordList;
                return this.$store.getters.getAccountingTableData;
            },
            perPageRecordList(){
            ...
            filterFields() {
                ...
                let hideColumns = ['original_invoice_number', 'closed_date', 'ship_date']
                let showListColumn = ['date', 'name_on_stone', 'title', 'invoice_number']
                const labelMap = {
                    date: 'Order Date',
            ...
            isCreatePermitted() {
                return this.$store.getters.isPermitted({resource: 'Accounting', action: 'create'});
            },
            isUpdatePermitted() {
                return this.$store.getters.isPermitted({resource: 'Accounting', action: 'update'});
            },
            isDeletePermitted() {
                return this.$store.getters.isPermitted({resource: 'Accounting', action: 'delete'});
            },
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                ...
                let tableNames = {
                    ...
                    'Accounting': {'columnTableName': 'accounting', 'childClassName': ''}
                };
            ...
            clearFilter: function () {
                ...
                if (this.sortByColumn == 'id' && this.sortDirection == 'desc' && this.sortByDesc == false && this.columnTableName == 'accounting')
                    this.filterRecords(1);
                this.sortByColumn = 'id'
                this.sortDirection = 'desc'
                this.sortByDesc = false
                this.columnTableName = 'accounting'
