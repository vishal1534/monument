<template>
    ...
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/composite-category-list">Back To List</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_COMPOSITE_CATEGORY);
            },
            first: function(){
                eventBus.$emit(event.FIRST_COMPOSITE_CATEGORY_RECORD);
            },
            last: function(){
                eventBus.$emit(event.LAST_COMPOSITE_CATEGORY_RECORD);
            },
            next: function(id){
                eventBus.$emit(event.NEXT_COMPOSITE_CATEGORY_RECORD, id);
            },
            prev: function(id){
                eventBus.$emit(event.PREVIOUS_COMPOSITE_CATEGORY_RECORD, id);
            },
            getRecord: function (value) {
                eventBus.$emit(event.GET_COMPOSITE_CATEGORY, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_COMPOSITE_CATEGORY);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_COMPOSITE_CATEGORY, id);
            }
        }
    }
</script>
