<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Task Collaboration Report</h5>
                    ...
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                ...
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!createdByDatePicker" v-model="createdByDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="createdByDatePicker" v-model="createdByDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #cell(close_date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.close_date ? changeDateFormat(data.item.close_date) : '' }}</span>
                            </template>
                        </b-table>
                    </div>
                </div>
        ...
        <PDF>
            <div class="pdf" slot="header">
                <h5 class="text-center">Task Collaboration Report</h5>
            </div>
            <div class="pdf" slot="body">
                <div class="row">
                    <b-table :fields="pdfTableFields" :items="tablePrintData"
                        ...
                        <template #cell(invoice_number)="data">
                            <span class="custom-table-body ml-1"> {{data.item.collaboration.order.invoice.invoice_number }}</span>
                        </template>
                        <template #cell(name_on_stone)="data">
                            <span class="custom-table-body ml-1"> {{ data.item.collaboration.order.family ? data.item.collaboration.order.family.name_on_stone : '' }}</span>
                        </template>
                        <template #cell(title)="data">
                            <span class="custom-table-body ml-1"> {{ data.item.department_id ? data.item.department.title : '' }}</span>
                        </template>
                        <template #cell(date)="data">
                            <span class="custom-table-body ml-1">{{ data.item.date ? changeDateFormat(data.item.date) : '' }}</span>
                        </template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                sortByDesc: false,
                columnTableName: 'idNotInChildOrParentTable',
                foreignKey: '',
                childClassName: '',
                ...
                queryFields: {},
                axiosParams: {module: 'collaborationTask'},
                pdfTableFields: [
                    ...
                    {key: 'assigned_to', label: "Assign To", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'task', label: "Task", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'title', label: "Dept", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'created_by', label: "Created By", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'close_date', label: "Close Date", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'status', label: "Task Status", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                ],
            }
        },
        metaInfo() {
            return {
                title: `Task Collaboration Report`,
            }
        },
        computed: {
            perPageRecordList() {
            ...
            tableData() {
                return this.$store.getters.getCollaborationTaskData;
            },
            tablePrintData() {
                return this.$store.getters.getCollaborationPrintTaskData;
            },
            isBusy() {
            ...
            filterFields() {
                ...
                const choosColumnLabel = ['Family', 'Invoice'];
                const labelMap = {
                    title: 'Dept', assigned_to: 'Assign To', status: 'Task Status', name_on_stone: 'Last Name On Stone'
                };
                ...
                const orderMap = [
                    {key: 'invoice_number', loc: 1, keyName: 'Invoice'},
                    {key: 'name_on_stone', loc: 2, keyName: 'Family'},
                    {key: 'date', loc: 3, keyName: 'CollaborationTask'},
                    {key: 'assigned_to', loc: 4, keyName: 'CollaborationTask'},
                    {key: 'task', loc: 5, keyName: 'CollaborationTask'},
                    {key: 'title', loc: 6, keyName: 'Department'},
                    {key: 'created_by', loc: 7, keyName: 'CollaborationTask'},
                    {key: 'close_date', loc: 8, keyName: 'CollaborationTask'},
                    {key: 'status', loc: 9, keyName: 'CollaborationTask'},
        ...
        methods: {
            ...
            filterRecords: async function (pageNo) {
                ...
                for (let key in this.queryFields) {
                    this.queryFields[key].filter(obj => {
                        ...
                        if (obj.operator == '...' && obj.column == 'close_date') {
                            isDateRangeSet = true
                            this.createdByDatePicker = false
                            if (this.createdByDate?.start && this.createdByDate?.end) {
                                obj.value = this.createdByDate.start + '...' + this.createdByDate.end
                                isDateRangeSet = false
                            } else if (this.orderDate == undefined) {
                                obj.value = null
                                isDateRangeSet = false
                            }
                        } else if (obj.operator != '...' && obj.column == 'close_date') {
                            this.createdByDatePicker = true
                            if (this.createdByDate?.start && this.createdByDate?.end) {
                                this.createdByDate = null
                            }
                            obj.value = this.createdByDate == undefined ? null : this.createdByDate
                        }
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'CollaborationTask': {'columnTableName': 'collaborationTask', 'childClassName': ''},
                    'Department': {'columnTableName': 'idInParentTable', 'foreignKey': 'department_id', 'childClassName': '\\Department'},
                ...
                if (this.sortByColumn != 'invoice_number' && tableNames[findParent.parent]) {
                    ...
                    } else if (tableNames[findParent.parent].tableName) {
                        this.childTableName = tableNames[findParent.parent].tableName;
                    }
                } else {
                    this.columnTableName = 'idNotInChildOrParentTable';
                    this.sortByColumn = 'invoice_number'
                    this.childTableName = 'invoices'
                }
            ...
            printPDF() {
                ...
                let queryData = [{
                    ...
                    'pagePerPage': this.pagePerPage, 'foreignKey': this.foreignKey, 'childClassName': this.childClassName,
                    'childTableName': this.childTableName, 'paginateOrGet': this.paginateOrGet,
                }]
                formData.append('queryData', JSON.stringify(queryData));
                this.axiosParams.filterData = formData;
...
/*
TODO:
 if you need to add setTimeOut then you can use nextTick which is more efficient then timeout.  Performance:
 nextTick leverages Vue's reactivity system and internal optimization mechanisms to schedule a callback after the next DOM update cycle.
 It ensures that the DOM has finished rendering and the component's state has been updated before executing the callback.
 This approach avoids unnecessary delays or unnecessary re-renders of the component.
 */
                    this.$nextTick(() => {
