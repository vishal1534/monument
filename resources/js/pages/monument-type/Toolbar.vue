<template>
    ...
    <div class="d-flex align-items-center w-100">
                    ...
                    <b-dropdown-group class="overflow-auto scroll input-md">
                        <div>
                            <div class="bg-toolbar text-center">
                                <label class="m-0">Monument Type</label>
                            </div>
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/monument-type-list">Back To List</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_MONUMENT_TYPE);
            },
            first: function(){
                eventBus.$emit(event.FIRST_MONUMENT_TYPE_RECORD);
            },
            last: function(){
                eventBus.$emit(event.LAST_MONUMENT_TYPE_RECORD);
            },
            next: function(id){
                eventBus.$emit(event.NEXT_MONUMENT_TYPE_RECORD, id);
            },
            prev: function(id){
                eventBus.$emit(event.PREVIOUS_MONUMENT_TYPE_RECORD, id);
            },
            getRecord: function (value) {
                eventBus.$emit(event.GET_MONUMENT_TYPE, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_MONUMENT_TYPE);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_MONUMENT_TYPE, id);
            }
        }
    }
</script>
