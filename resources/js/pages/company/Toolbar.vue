<template>
    <div class="d-flex align-items-center w-100">
        <div class="toolbar" v-if="isReadPermitted || isUpdatePermitted">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/company-list">Back To List</router-link>
            <button class="btn-info mx-1 rounded custom-infobtn-text" type="button">
                Print Customer Info.
            </button>
        </div>
        <div class="toolbar ml-auto">
...
<script>
    ...
    export default {
        ...
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_COMPANY);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_CUSTOMER_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_CUSTOMER_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_CUSTOMER_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_CUSTOMER_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_CUSTOMER, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_CUSTOMER);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_CUSTOMER, id);
            },
        }
    }
</script>
