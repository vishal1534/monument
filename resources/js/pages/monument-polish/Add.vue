<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Monument Polish</h5>
        ...
        <div class="row m-0">
        <div id="monumentType" class="col-md-12">

            <form id="monument-form">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Monument Polish Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
                        <label class="font-weight-bold">Title:</label>
                        <b-form-input type="text" :disabled="hideInModal || hideinAuth" name="product_polish_title" class="input-xl" size="sm" v-model="formFields.product_polish_title"></b-form-input>
                    </div>
                </div>
                    </div>
    ...
    export default {
        ...
        data() {
            return  {
                formFields:{
                    id:0,
                    product_polish_title: null,
                },
                axiosParams: {module: 'monumentPolish'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Monument Polish`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_MONUMENT_POLISH, this.storeMonumentPolish);
            eventBus.$on(event.FIRST_MONUMENT_POLISH_RECORD, this.first);
            eventBus.$on(event.LAST_MONUMENT_POLISH_RECORD, this.last);
            eventBus.$on(event.NEXT_MONUMENT_POLISH_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_MONUMENT_POLISH_RECORD, this.prev);
            eventBus.$on(event.GET_MONUMENT_POLISH, this.getMonumentPolish);
            eventBus.$on(event.ADD_MONUMENT_POLISH, this.clearForm);
            eventBus.$on(event.DELETE_MONUMENT_POLISH, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_MONUMENT_POLISH, this.storeMonumentPolish);
            eventBus.$off(event.FIRST_MONUMENT_POLISH_RECORD, this.first);
            eventBus.$off(event.LAST_MONUMENT_POLISH_RECORD, this.last);
            eventBus.$off(event.NEXT_MONUMENT_POLISH_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_MONUMENT_POLISH_RECORD, this.prev);
            eventBus.$off(event.GET_MONUMENT_POLISH, this.getMonumentPolish);
            eventBus.$off(event.ADD_MONUMENT_POLISH, this.clearForm);
            eventBus.$off(event.DELETE_MONUMENT_POLISH, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getMonumentPolish(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getMonumentPolish: function(id){
            ...
            storeMonumentPolish: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Monument Polish '+status, message : this.alertMessage, variant : 'success'});
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                            if (this.$store.state[this.axiosParams.module].recordList.length == 0){
                                ...
                                let params = {module: 'monument', detail: 'side', id: this.axiosParams.id}
