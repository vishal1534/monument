<template>
    ...
    <div v-if="isCreatePermitted || isUpdatePermitted" class="d-flex align-items-center w-100">
                    ...
                    <b-dropdown-group class="overflow-auto scroll input-md">
                        <div>
                            <div class="bg-toolbar text-center">
                                <label class="m-0">Monument Side</label>
                            </div>
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/monument-polish-list">Back To List</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            recordList: function () {
                ...
                for (let opt of this.$store.state[this.axiosParams.module].recordList){
                    let obj = {value: opt.id, text: opt.product_polish_title};
                    options.push(obj);
                }
                if (this.searchTerm != ''){
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_MONUMENT_POLISH);
            },
            first: function(){
                eventBus.$emit(event.FIRST_MONUMENT_POLISH_RECORD);
            },
            last: function(){
                eventBus.$emit(event.LAST_MONUMENT_POLISH_RECORD);
            },
            next: function(id){
                eventBus.$emit(event.NEXT_MONUMENT_POLISH_RECORD, id);
            },
            prev: function(id){
                eventBus.$emit(event.PREVIOUS_MONUMENT_POLISH_RECORD, id);
            },
            getRecord: function (value) {
                eventBus.$emit(event.GET_MONUMENT_POLISH, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_MONUMENT_POLISH);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_MONUMENT_POLISH, id);
            },
        }
    }
</script>
