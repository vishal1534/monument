<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Monument Type</h5>
            ...
            <form id="monument-form">
               ...
               <div class="pt-1">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Monument Type Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
                        <label class="font-weight-bold">Title:</label>
                        <b-form-input type="text" :disabled="hideInModal || hideinAuth" name="type_title" class="input-xl" size="sm" v-model="formFields.type_title"></b-form-input>
                    </div>
                </div>
                </div>
            </form>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'monumentType'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Monument Type`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_MONUMENT_TYPE, this.storeMonumentType);
            eventBus.$on(event.FIRST_MONUMENT_TYPE_RECORD, this.first);
            eventBus.$on(event.LAST_MONUMENT_TYPE_RECORD, this.last);
            eventBus.$on(event.NEXT_MONUMENT_TYPE_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_MONUMENT_TYPE_RECORD, this.prev);
            eventBus.$on(event.GET_MONUMENT_TYPE, this.getMonumentType);
            eventBus.$on(event.ADD_MONUMENT_TYPE, this.clearForm);
            eventBus.$on(event.DELETE_MONUMENT_TYPE, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_MONUMENT_TYPE, this.storeMonumentType);
            eventBus.$off(event.FIRST_MONUMENT_TYPE_RECORD, this.first);
            eventBus.$off(event.LAST_MONUMENT_TYPE_RECORD, this.last);
            eventBus.$off(event.NEXT_MONUMENT_TYPE_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_MONUMENT_TYPE_RECORD, this.prev);
            eventBus.$off(event.GET_MONUMENT_TYPE, this.getMonumentType);
            eventBus.$off(event.ADD_MONUMENT_TYPE, this.clearForm);
            eventBus.$off(event.DELETE_MONUMENT_TYPE, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        computed:{
            ...
            categories: function () {
                ...
                let options = [];
                for (let opt of this.$store.state.service.serviceInfo){
        ...
        mounted() {
            ...
            this.getMonumentType(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getMonumentType: function(id){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                       ...
                       this.alertMessage = '';
                        ...
                        this.makeToast({title : 'Monument Type '+status, message : this.alertMessage, variant : 'success'});
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                            if (this.$store.state[this.axiosParams.module].recordList.length == 0){
                                ...
                                let params = {module: 'monument', detail: 'type', id: this.axiosParams.id}
