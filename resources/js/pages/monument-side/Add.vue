<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Monument Side</h5>
        ...
        <div id="monumentType" class="col-md-12">
            ...
            <form id="monument-form">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Monument Side Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
                        <label class="font-weight-bold">Title:</label>
                        <b-form-input type="text" :disabled="hideInModal || hideinAuth" name="side_title" class="input-xl" size="sm" v-model="formFields.side_title"></b-form-input>
                    </div>
                </div>
                </div>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'monumentSide'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Monument Side`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_MONUMENT_SIDE, this.storeMonumentSide);
            eventBus.$on(event.FIRST_MONUMENT_SIDE_RECORD, this.first);
            eventBus.$on(event.LAST_MONUMENT_SIDE_RECORD, this.last);
            eventBus.$on(event.NEXT_MONUMENT_SIDE_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_MONUMENT_SIDE_RECORD, this.prev);
            eventBus.$on(event.GET_MONUMENT_SIDE, this.getMonumentSide);
            eventBus.$on(event.ADD_MONUMENT_SIDE, this.clearForm);
            eventBus.$on(event.DELETE_MONUMENT_SIDE, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_MONUMENT_SIDE, this.storeMonumentSide);
            eventBus.$off(event.FIRST_MONUMENT_SIDE_RECORD, this.first);
            eventBus.$off(event.LAST_MONUMENT_SIDE_RECORD, this.last);
            eventBus.$off(event.NEXT_MONUMENT_SIDE_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_MONUMENT_SIDE_RECORD, this.prev);
            eventBus.$off(event.GET_MONUMENT_SIDE, this.getMonumentSide);
            eventBus.$off(event.ADD_MONUMENT_SIDE, this.clearForm);
            eventBus.$off(event.DELETE_MONUMENT_SIDE, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getMonumentSide(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getMonumentSide: function(id){
            ...
            storeMonumentSide: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Monument Side '+status, message : this.alertMessage, variant : 'success'});
