<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Cost of Production Inventory</h5>
...
<!--                        foot-clone-->
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :per-page="pagePerPage" bordered class="my-table bootstrap-vue-table a" hover
                                 id="data-table" no-footer-sorting responsive small striped>
                            <template #table-busy>
                            ...
                            <template #cell(total_added)="data">
                                <span class="custom-table-body ml-1">$ {{  convertDecimal(data.item.total_on_hand * data.item.product_cost ) }}</span>
                            </template>
                           <!-- <template #foot()="data">
                            ...
                            <template #foot(total_on_hand)="data">
                                <label>Total : {{ convertDecimal(Number(calculateSubTotal.total_on_hand)) }}</label>
                            </template>-->
                        </b-table>
                    </div>
                </div>
        ...
        <PDF>
            <div class="pdf" slot="header">
                <h5 class="text-center">Cost of Production Inventory</h5>
            </div>
            <div class="pdf" slot="body">
                <div class="row">
                    <b-table :fields="pdfTableFields" :items="tableData" bordered class="" foot-clone id="data-table-pdf" no-footer-sorting
                        ...
                        <template #foot(total_on_hand)="data">
                            <label>{{ convertDecimal(Number(calculateAllSubTotal.total_on_hand)) }}</label>
                        </template>
                    </b-table>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        metaInfo() {
            return {
                title: `Cost of Production Inventory List`,
            }
        },
        computed: {
            perPageRecordList(){
            ...
            filterFields() {
                ...
                const hideListColumn = ['cost', 'product_type','description','gi_misc_note','gi_misc_note_a'];
                const showListColumn = ['product_number', 'item_name', 'color_title', 'type_title', 'total_on_hand',
                    'product_cost','total_added'];
                const labelMap = {
                    color_title: 'Color', type_title: 'Type', total_on_hand: 'QTY', product_cost: 'Cost', total_added: 'Total',
                };
                ...
                const orderMap = [
                    ...
                    {key: 'product_cost', loc: 7},
                    {key: 'total_added', loc: 8},
                ];
            ...
            calculateSubTotal() {
                ...
                for (startPoint; startPoint < endPoint; startPoint++) {
                    if (record[startPoint]) {
                        ...
                        fields.total_added += Number(record[startPoint]['total_on_hand']) * Number(record[startPoint]['product_cost'])
                        fields.total_on_hand += record[startPoint]['total_on_hand']
                    }
                }
                return fields;
            },
