<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Service Category</h5>
        ...
        <div id="monumentType" class="col-md-12">
            ...
            <form id="monument-form">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Service Category Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'serviceCategory'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Service Category`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_SERVICE_CATEGORY, this.storeServiceCategory);
            eventBus.$on(event.FIRST_SERVICE_CATEGORY_RECORD, this.first);
            eventBus.$on(event.LAST_SERVICE_CATEGORY_RECORD, this.last);
            eventBus.$on(event.NEXT_SERVICE_CATEGORY_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_SERVICE_CATEGORY_RECORD, this.prev);
            eventBus.$on(event.GET_SERVICE_CATEGORY, this.getServiceCategory);
            eventBus.$on(event.ADD_SERVICE_CATEGORY, this.clearForm);
            eventBus.$on(event.DELETE_SERVICE_CATEGORY, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_SERVICE_CATEGORY, this.storeServiceCategory);
            eventBus.$off(event.FIRST_SERVICE_CATEGORY_RECORD, this.first);
            eventBus.$off(event.LAST_SERVICE_CATEGORY_RECORD, this.last);
            eventBus.$off(event.NEXT_SERVICE_CATEGORY_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_SERVICE_CATEGORY_RECORD, this.prev);
            eventBus.$off(event.GET_SERVICE_CATEGORY, this.getServiceCategory);
            eventBus.$off(event.ADD_SERVICE_CATEGORY, this.clearForm);
            eventBus.$off(event.DELETE_SERVICE_CATEGORY, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getServiceCategory(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            ...
            storeServiceCategory: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Servive Category '+status, message : this.alertMessage, variant : 'success'});
                ...
                if (confirm("Do you really want to delete this record?")){
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                                ...
                                let params = {module: 'service', detail: 'category', id: this.axiosParams.id}
