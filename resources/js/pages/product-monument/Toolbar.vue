<template>
    <div class="d-flex align-items-center w-100">
                ...
                <b-dropdown menu-class="p-0 m-0 custom-dropdown-ul" no-caret size="sm" variant="none">
                    <template #button-content>
                        <div class="d-flex input-md form-control-sm rounded-0 custom-caret-toolbar my-1 ml-n2">
                            <label class="overflow-hidden h-100 label-input-width">{{getSelected[0] ? getSelected[0].text : 'Go To' }}</label>
                        </div>
                    </template>
                    <b-dropdown-group class="overflow-auto scroll input-md">
                        <div>
                            <div class="bg-toolbar text-center">
                                <label class="m-0">Monument</label>
                            </div>
        ...
        <div class="toolbar ml-auto">
            ...
            <template v-if="!isCurrentPageURL">
                <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/monuments-list">Back To List</router-link>
                <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/product-catalog-list">Navigate To All Product List</router-link>
            </template>
            <template v-else-if="isCurrentPageURL">
                <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/price-estimator-list">Back To List</router-link>
                <router-link :to="'/quotes/' + quote_id" class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" :class="quote_id ? '' : 'd-none'">Back To Quote</router-link>
            </template>
        </div>
        <div class="toolbar ml-auto">
...
<script>
    ...
    export default {
        ...
        computed: {
            recordList: function () {
                let options = [];
                if (this.pageURL != 'price-estimator') {
                    this.$store.getters.sortByAlphaNumeric({'module': 'monument', 'fieldName': 'product_number'});
                    for (let opt of this.$store.state[this.axiosParams.module].recordList) {
                        let obj = {value: opt.id, text: opt.product_number};
                        options.push(obj);
                    }
                } else {
                    this.$store.getters.sortByAlphaNumeric({'module': 'monument', 'fieldName': 'item_name'});
                    for (let opt of this.$store.state[this.axiosParams.module].recordList) {
                        let obj = {value: opt.id, text: opt.item_name};
                        options.push(obj);
                    }
                }
                if (this.searchTerm != '') {
                    options = options.filter(obj => obj.text ? obj.text.toUpperCase().indexOf(this.searchTerm.toUpperCase()) > -1 : '')
                }
                return options;
            },
            ...
            isCurrentPageURL() {
                return this.pageURL == 'price-estimator'
            },
        },
        data() {
            return {
        ...
        mounted() {
            this.pageURL = this.$route.path.split('/')[1];
        },
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_MONUMENT);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_MONUMENT_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_MONUMENT_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_MONUMENT_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_MONUMENT_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_MONUMENT, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_MONUMENT);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_MONUMENT, id);
            }
        }
    }
</script>
