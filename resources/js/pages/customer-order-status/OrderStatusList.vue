        ...
        <div class="ml-4 mr-4">
            ...
            >
                ...
                <template #cell(status)="data">
                    <span class="custom-table-body ml-1"> {{ data.item.order_status != null ? customizeLabel(data.item.order_status.title) : '' }}</span>
                </template>

                <template #cell(action)="data">
                    <div class="mx-1 custom-input-header">
                        <b-button :to="'customer-order-status/' + data.item.id" class="btn mx-1 py-1 custom-view-btn custom-view-text" v-if="isReadPermitted">View</b-button>
    ...
    export default {
        ...
        computed: {
            ...
            filterFields() {
                ...
                const showListColumn = ['date', 'name_on_stone', 'title', 'invoice_number', 'date_promised'];
                const hideListColumn = ['original_invoice_number', 'ship_date', 'closed_date'];
                const labelMap = {
                    date: 'Order Date', name_on_stone: 'Last Name on Stone', invoice_number: 'Invoice No.', title: 'Order Status'
                };
        ...
        methods: {
            ...
            getColumnList: function () {
                if (this.$store.state[this.axiosParams.module].columnList.length == 0) {
                    this.axiosParams.column = 'productionColumn';
            ...
            getRecordList: function () {
                if (this.$store.state[this.axiosParams.module].recordList.length == 0) {
                    this.axiosParams.methodName = 'orderStatusInfo';
