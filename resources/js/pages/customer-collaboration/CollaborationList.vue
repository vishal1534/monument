<template>
    <div>
        ...
        <div class="ml-4 mr-4">
            <div class="d-flex">
                <b-dropdown
                    class="m-2 custom-choose-btn d-none" id="dropdown-form" ref="dropdown" size="sm" text="Choose Columns" variant="info custom-choose-text">
            ...
            <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :per-page="perPage" bordered
                ...
                <template #cell(invoice_total)="data">
                    <span class="custom-table-body ml-1">$ {{ data.item.order.invoice ? data.item.order.invoice.invoice_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                </template>

                <template #cell(action)="data">
                    <div class="mx-1 custom-input-header">
                        <b-button :to="'customer-collaboration/' + data.item.id" class="btn mx-1 py-1 custom-view-btn custom-view-text" v-if="isReadPermitted">View</b-button>
        ...
        methods: {
            ...
            getColumnList: function () {
                if (this.$store.state[this.axiosParams.module].columnList.length == 0) {
                    this.axiosParams.column = 'collaborationColumn';
