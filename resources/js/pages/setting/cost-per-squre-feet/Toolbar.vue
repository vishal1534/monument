<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/cost-per-squre-feet-list">Back To List</router-link>
        </div>
        <div class="toolbar ml-auto">
...
<script>
    ...
    export default {
        ...
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_COSTPSQUREFEET);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_COSTPSQUREFEET_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_COSTPSQUREFEET_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_COSTPSQUREFEET_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_COSTPSQUREFEET_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_COSTPSQUREFEET, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_COSTPSQUREFEET);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_COSTPSQUREFEET, id);
            }
        }
    }
</script>
