<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/weekly-schedule-list">Cancel</router-link>
        </div>
        <div class="toolbar ml-auto">
    ...
    export default {
        ...
        methods: {
            ...
            first: function () {
                ...
                eventBus.$emit(event.FIRST_WEEKLY_SCHEDULE_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_WEEKLY_SCHEDULE_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_WEEKLY_SCHEDULE_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_WEEKLY_SCHEDULE_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_WEEKLY_SCHEDULE, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_WEEKLY_SCHEDULE);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_WEEKLY_SCHEDULE, id);
            }
        }
    }
</script>
