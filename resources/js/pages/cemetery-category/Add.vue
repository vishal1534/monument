<template>
    <div>
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Cemetery Category Information</h5>
                    ...
                    <form >
                        ...
                        <div class="div-shadow bg-white rounded p-2 mb-2">
                            ...
                            <div class="company-row">
                                <div>
                                    <h6 class="my-auto ml-2 custom-form-heading">Cemetery Category Information</h6>
                                </div>
                            </div>
                            ...
                            <div class="company-row">
                                <div>
                                    ...
                                    <b-form-input type="text" :disabled="hideInModal" class="input-xl" name="name" size="sm" v-model="formFields.title"></b-form-input>
                                </div>
                            </div>
                        </div>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'cemeteryCategory'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Cemetery Category`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_CEMETERY_CATEGORY, this.storeCemeteryCategory);
            eventBus.$on(event.FIRST_CEMETERY_CATEGORY_RECORD, this.first);
            eventBus.$on(event.LAST_CEMETERY_CATEGORY_RECORD, this.last);
            eventBus.$on(event.NEXT_CEMETERY_CATEGORY_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_CEMETERY_CATEGORY_RECORD, this.prev);
            eventBus.$on(event.GET_CEMETERY_CATEGORY, this.getCemeteryCategory);
            eventBus.$on(event.ADD_CEMETERY_CATEGORY, this.clearForm);
            eventBus.$on(event.DELETE_CEMETERY_CATEGORY, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_CEMETERY_CATEGORY, this.storeCemeteryCategory);
            eventBus.$off(event.FIRST_CEMETERY_CATEGORY_RECORD, this.first);
            eventBus.$off(event.LAST_CEMETERY_CATEGORY_RECORD, this.last);
            eventBus.$off(event.NEXT_CEMETERY_CATEGORY_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_CEMETERY_CATEGORY_RECORD, this.prev);
            eventBus.$off(event.GET_CEMETERY_CATEGORY, this.getCemeteryCategory);
            eventBus.$off(event.ADD_CEMETERY_CATEGORY, this.clearForm);
            eventBus.$off(event.DELETE_CEMETERY_CATEGORY, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getCemeteryCategory(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getCemeteryCategory: function(id){
            ...
            storeCemeteryCategory: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data[0]){
                        ...
                        this.makeToast({title : 'Cemetery Category '+status, message : this.alertMessage, variant : 'success'});
            ...
            /** Generic*/
            clearForm: function(){
                this.formFields = {id:0,generic:0,notify:[]};
            },
