<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar">
            <button @click="addRecord" class="btn btn-sm custom-label-toolbar" type="button">New Record</button>
            <button :disabled="emptyRecord" @click="deleteRecord(id)" class="btn btn-sm custom-label-toolbar" type="button">Delete Record</button>
        </div>
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/user-list">Back To List</router-link>
        </div>
        <div class="toolbar ml-auto">
            <div class="d-flex">
...
<script>
    ...
    export default {
        ...
        computed: {
            recordList: function () {
                let options = [];
                for (let opt of this.$store.state[this.axiosAddEditParams.module].recordList) {
            ...
            totalRecords: function () {
                return this.$store.state[this.axiosAddEditParams.module].recordList.length;
            },
            currentRecord: function () {
                let list = this.$store.state[this.axiosAddEditParams.module].recordList;
                let record = list.find(obj => obj.id == this.id);
                let currentIndex = list.indexOf(record) + 1;
                return currentIndex
            },
            emptyRecord: function () {
                return this.$store.state[this.axiosAddEditParams.module].recordList.length == 0 || this.$props.id == 0;
            },
            isCreatePermitted() {
        ...
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_USER);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_USER_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_USER_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_USER_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_USER_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_USER, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_USER);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_USER, id);
            },
        }
    }
</script>
