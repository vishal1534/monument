<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Installation Production List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <router-link class="btn custom-infobtn-list custom-infobtn-list-text ml-4" to="/installation-production" v-if="isCreatePermitted">Add New</router-link>
                <div class="d-flex m-auto">
                    <button class="btn-info mx-1 custom-infobtn-toolbar custom-infobtn-text" type="button" v-if="false">Print List ?</button>
                </div>
                <div class="d-flex m-auto">
                    <router-link :to="'/installation-list'" class="btn mx-1 py-1 custom-view-btn custom-view-text">Back to Installation</router-link>
                </div>
                <div class="d-flex">
                </div>
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'installation-production/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'installationProduction'" :recordID="viewRecord" :title="'Installation Production'"></Modal>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                sortByColumn: 'id',
                sortByDesc: false,
                columnTableName: 'installationProduction',
                foreignKey: '',
                childClassName: '',
                childTableName: '',
                sortDirection: 'desc',
        ...
        metaInfo() {
            return {
                title: `Installation Production List`,
            }
        },
        computed: {
            perPageRecordList() {
            ...
            tableData() {
                return this.$store.getters.getInstallationProductionTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Installation Production': {'columnTableName': 'installationProduction', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'installationProduction';
