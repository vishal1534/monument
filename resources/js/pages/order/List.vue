<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Order List</h5>
        <div class="bg-toolbar my-2 p-1">

            <div class="d-flex justify-content-end align-items-center">

                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text mx-3" to="/orders" v-if="isCreatePermitted">Add Order</router-link>
                    <!--                    <button :disabled="toggleSyncOrderBtn" class="btn custom-infobtn-list custom-infobtn-list-text" @click="syncOrderList">
                                            Sync Orders
                                            <b-spinner v-if="toggleSyncOrderBtn" small class="custom-spinner-btn" type="grow"></b-spinner>
                                        </button>-->
                ...
                <div class="col-md-12">
                    ...
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                 ...
                                 bordered
                                 class="my-table bootstrap-vue-table a"
                                 hover
                                 id="data-table"
                                 responsive
                                 small
                            ...
                            <template #head(is_installation)="data">
                                <div class="d-flex mx-1 custom-input-header w-100">
                                    <div class="btn-group">
                            ...
                            <template #head(date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <!--<b-form-datepicker :date-format-options="{ year: 'numeric', month: 'numeric', day: 'numeric' }" class="form-control-sm rounded-0 border" size="sm" @input="filterRecords" placeholder="" v-model="queryFields[data.field.parent].find(item => item.column == data.column).value"
                                                            today-button
                                                            reset-button
                                                            close-button
                                                            locale="en">
                                        ...
                                        <!--Reference of date picker range  https://vue-datepicker.netlify.app/ -->
                            ...
                            <template #cell(ship_date)="data">
                                <span class="custom-table-body ml-1"> {{ changeDateFormat(data.item.invoice.ship_date) }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'orders/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                            ...
                            <template #cell(sub_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.sub_total ? data.item.sub_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(discount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.discount ? data.item.discount.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(invoice_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.invoice_total ? data.item.invoice_total.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(tax_amount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.tax_amount ? data.item.tax_amount.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(tax_rate)="data">
                                <span class="custom-table-body ml-1">% {{ data.item.tax_rate ? data.item.tax_rate.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>

                            <template #cell(order_status_id)="data">
                                <span class="custom-table-body ml-1"> {{ data.item.order_status_id ? customizeLabel(data.item.order_status.title) : '' }}</span>
                            </template>

                        </b-table>

                        <Modal :module="'Order'" :recordID="viewRecord" :title="'Order'"></Modal>
...
<script>
    import {deleteRecord, fetchPaginateRecord, filterRecords, syncOrders} from '../../helpers/axios';
    ...
    export default {
        ...
        computed: {
            ...
            filterFields() {
                ...
                const showListColumn = ['date', 'name_on_stone', 'title', 'invoice_number', 'closed_date'];
                const hideListColumn = ['date_promised', 'ship_date'];
                ...
                const dropDownKeyMap = [
                    ...
                    {key: 'tax_rate', loc: 11, keyName: 'Inv/Order'},
                    {key: 'discount', loc: 12, keyName: 'Inv/Order'},
                ];
        ...
        methods: {
                ...
                if (confirm("Do you really want to delete this record?")) {
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data) {
                            ...
                            let params = {module: 'order', details: ['production', 'collaboration', 'installation'], id: this.axiosParams.id}
                            this.$store.dispatch('removeOrderDetailFromList', params);
                        } else {
            ...
            filterRecords: async function (pageNo) {
                ...
                // formData.append('sortDirection', JSON.stringify(this.sortDirection));
            ...
            syncOrderList: function () {
                this.toggleSyncOrderBtn = true;
                syncOrders(this.axiosParams, (response) => {
                    if (response.data.errorInfo) {
                        this.makeToast({title: 'Error', message: response.data.errorInfo, variant: 'danger'});
                    } else if (response.data.warnInfo) {
                        this.makeToast({title: 'Error', message: response.data.warnInfo, variant: 'warn'});
                    } else {
                        this.$store.dispatch('getModuleInfo', this.axiosParams);
                        let message = 'Orders Synced Successfully!';
                        this.makeToast({title: 'Success', message: message, variant: 'success'});
                    }
                    this.toggleSyncOrderBtn = false;
                }, (error) => {
                    this.makeToast({title: 'Error', message: error.response.data.message, variant: 'danger', duration: 3000});
                    this.toggleSyncOrderBtn = false;
                })
            ...
            sortByAscDesc: function () {
                ...
                let tableNames = {
                    ...
                    'Order Status': {'columnTableName': 'idInParentTable', 'foreignKey': 'order_status_id', 'childClassName': '\\OrderStatus'},
                    'Order Type': {'columnTableName': 'idInParentTable', 'foreignKey': 'order_type_id', 'childClassName': '\\OrderType'},
                    'Paper Location': {'columnTableName': 'idInParentTable', 'foreignKey': 'paper_location_id', 'childClassName': '\\PaperLocation'},
                    'Order': {'columnTableName': 'order', 'childClassName': ''}
                };
