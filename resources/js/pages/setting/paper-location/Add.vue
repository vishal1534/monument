<template>
    <div>
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Paper Location Information</h5>
                        ...
                        <template v-else>
                            <div class="div-shadow bg-white rounded p-2 mb-2">
                                ...
                                <div class="company-row">
                                    <div>
                                        <h6 class="my-auto ml-2 custom-form-heading">Paper Location Information</h6>
                                    </div>
                                </div>
                                <div class="company-row">
                                    <div>
                                        <label class="font-weight-bold">Title:</label>
                                        <b-form-input :disabled="hideInModal" class="input-xl" name="location_title" size="sm" type="text" v-model="formFields.location_title"></b-form-input>
                                    </div>
                                </div>
                            </div>
                        </template>
...
<script>
    ...
    export default {
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Paper Location`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_PAPER_LOCATION, this.storePaperLocation);
            eventBus.$on(event.FIRST_PAPER_LOCATION_RECORD, this.first);
            eventBus.$on(event.LAST_PAPER_LOCATION_RECORD, this.last);
            eventBus.$on(event.NEXT_PAPER_LOCATION_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_PAPER_LOCATION_RECORD, this.prev);
            eventBus.$on(event.GET_PAPER_LOCATION, this.getPaperLocation);
            eventBus.$on(event.ADD_PAPER_LOCATION, this.clearForm);
            eventBus.$on(event.DELETE_PAPER_LOCATION, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_PAPER_LOCATION, this.storePaperLocation);
            eventBus.$off(event.FIRST_PAPER_LOCATION_RECORD, this.first);
            eventBus.$off(event.LAST_PAPER_LOCATION_RECORD, this.last);
            eventBus.$off(event.NEXT_PAPER_LOCATION_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_PAPER_LOCATION_RECORD, this.prev);
            eventBus.$off(event.GET_PAPER_LOCATION, this.getPaperLocation);
            eventBus.$off(event.ADD_PAPER_LOCATION, this.clearForm);
            eventBus.$off(event.DELETE_PAPER_LOCATION, this.deleteRecord);
        },
        components: {
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getPaperLocation(this.formFields.id);
            this.getColumns();
            this.getRecords();
            ...
            storePaperLocation: function () {
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data[0]) {
                        ...
                        this.makeToast({title: 'Paper Location ' + status, message: this.alertMessage, variant: 'success'});
                        this.formFields = response.data;
                        this.alertMessage = '';
