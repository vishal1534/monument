<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Reorder Quantity</h5>
                    ...
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :per-page="pagePerPage" bordered
                            ...
                            <template #cell(total_added)="data">
                                <span class="custom-table-body ml-1">$ {{  convertDecimal(data.item.product_cost * (data.item.quantity_stocked - data.item.total_on_hand)) }}</span>
                            </template>
                            <!-- search filter will not work with this column total_sold (Reorder) -->
                            <template #cell(total_sold)="data">
                                <span class="custom-table-body ml-1"> {{  data.item.quantity_stocked - data.item.total_on_hand  }}</span>
                            </template>
                            <!--<template #foot()="data">
                            ...
                            <template #foot(total_sold)="data">
                                <label>Total : {{ convertDecimal(calculateSubTotal.total_sold) }}</label>
                            </template>-->
                        </b-table>
                    </div>
                </div>
        ...
        <PDF>
            <div class="pdf" slot="header">
                <h5 class="text-center">Reorder Quantity</h5>
            </div>
            <div class="pdf" slot="body">
                <div class="row">
                        <b-table :fields="pdfTableFields" :items="tableData" bordered class="" foot-clone id="data-table-pdf" no-footer-sorting
                            ...
                            <template #foot(total_sold)="data">
                                <label>{{ convertDecimal(calculateAllSubTotal.total_sold) }}</label>
                            </template>
                        </b-table>
                </div>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                pdfTableFields: [
                    ...
                    {key: 'quantity_stocked', label: "QTY Stocked", class: 'small', thClass: 'text-nowrap', tdClass: ''},
                    {key: 'total_sold', label: "Reorder", class: 'small', thClass: 'text-nowrap', tdClass: ''},
        ...
        computed: {
            ...
            filterFields() {
                ...
                for (const [key, value] of Object.entries(this.$store.state[this.axiosParamsReport.module].columnList)) {
                    ...
                    for (let val of value) {
                        ...
                        if (val == 'cost' || val == 'description' || val == 'gi_misc_note' || val == 'product_type' || val == 'gi_misc_note_a') {
                            showColumn = 'd-none';
                        }
                        if (val == 'product_number' || val == 'item_name' || val == 'color_title' || val == 'total_sold' || val == 'total_added'
                            || val == 'type_title' || val == 'total_on_hand' || val == 'quantity_stocked' || val == 'product_cost') {
                            showColumn = '';
                        }
                        let label = this.customizeLabel(val);
                        if (val == 'color_title') {
                            label = 'Color';
                        } else if (val == 'type_title') {
                            label = 'Type';
                        } else if (val == 'total_on_hand') {
                            label = 'QTY on Hand'
                        } else if (val == 'quantity_stocked') {
                            label = 'QTY Stocked'
                        } else if (val == 'total_sold') {
                            label = 'Reorder'
                        } else if (val == 'product_cost') {
                            label = 'Cost'
                        } else if (val == 'total_added') {
                            label = 'Total'
                        }
                ...
                const orderMap = [
                    ...
                    {key: 'total_on_hand', loc: 6},
                    {key: 'quantity_stocked', loc: 7},
                    {key: 'total_sold', loc: 8},
                    {key: 'product_cost', loc: 9},
                    {key: 'total_added', loc: 10},
                ];
            ...
            calculateSubTotal() {
                ...
                for (startPoint; startPoint < endPoint; startPoint++) {
                    if (record[startPoint]) {
                        ...
                        fields.total_added += record[startPoint]['product_cost'] * (record[startPoint]['quantity_stocked'] - record[startPoint]['total_on_hand'])
                        fields.total_sold += record[startPoint]['quantity_stocked'] - record[startPoint]['total_on_hand']
                    }
                }
                return fields;
            },
            calculateAllSubTotal() {
                let fields = {
                    product_cost: 0, total_added: 0, total_sold: 0
                };
                let record = this.$store.state[this.axiosParamsReport.module].recordList;
                let startPoint = 0;
