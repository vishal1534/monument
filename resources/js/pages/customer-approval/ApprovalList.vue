<template>
    <div>
        ...
        <div class="ml-4 mr-4">
            ...
            <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :per-page="perPage" bordered
                     class="my-table bootstrap-vue-table a" hover responsive small striped>
                <template #table-busy>
                ...
                <template #head(contact)="data">
                    <div class="d-flex mx-1 custom-input-header">
                        <div class="btn-group">
                            <select @change="filterRecords" class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).value">
                ...
                <template #head(date)="data">
                    <div class="d-flex mx-1 custom-date-header">
                        <div class="btn-group">
                            <select @change="filterRecords" class="form-control-sm rounded-0 border bg-light"
                                    v-model="queryFields[data.field.parent].find(item => item.column == data.column).operator">
                ...
                <template #head()="data">
                    <div class="d-flex mx-1 custom-input-header">
                        <div class="mx-1 btn-group">
                            ...
                            <input @input="debounce" @keyup.enter="filterRecords" class="form-control-sm border rounded-0 custom-input-width"
                                   placeholder="Search here..." type="text" v-model="queryFields[data.field.parent].find(item => item.column == data.column).value"/>
                        </div>
                    </div>
                    <hr class="border">
                    <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                ...
                <template #cell(date)="data">
                    <span class="custom-table-body ml-1"> {{ changeDateFormat(data.item.order.date) }}</span>
                </template>

                <template #cell(contact)="data">
                    <span class="custom-table-body ml-1">{{ data.item.images.length > 0 ? 'Yes' : 'No' }}</span>
                </template>

                <template #cell(action)="data">
                    <div class="mx-1 custom-input-header">
                        <b-button :to="'customer-approval/' + data.item.id" class="btn mx-1 py-1 custom-view-btn custom-view-text" v-if="isReadPermitted && data.item.images.length > 0">View</b-button>
    ...
    export default {
        ...
        computed: {
            ...
            filterFields() {
                ...
                const showListColumn = ['date', 'name_on_stone', 'contact', 'invoice_number'];
                const hideListColumn = ['ship_date', 'closed_date', 'date_promised'];
                const labelMap = {
                    date: 'Order Date', name_on_stone: 'Last Name on Stone', invoice_number: 'Invoice No.', contact: 'Approval Needed?'
                };
                ...
                if (Object.keys(this.queryFields).length > 0) {
                    let images = [{column: 'contact', operator: "like", value: ""}];
                    this.queryFields['image'] = images;
                }
                const orderMap = [
        ...
        methods: {
            ...
            getColumnList: function () {
                if (this.$store.state[this.axiosParams.module].columnList.length == 0) {
                    this.axiosParams.column = 'approvalColumn';
            ...
            getRecordList: function () {
                if (this.$store.state[this.axiosParams.module].recordList.length == 0) {
                    this.axiosParams.methodName = 'approvalInfo';
