<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            <button :disabled="saveDisabled" @click="save" class="btn-info mx-1 w-auto px-3 custom-infobtn-toolbar custom-infobtn-text" type="button" v-if="id ? isUpdatePermitted : isCreatePermitted">
                {{ id ? 'Update' : 'Save' }}<span class="spinner-grow spinner-grow-sm ml-1" v-if="saveDisabled"></span>
            </button>
            <router-link class="btn btn-sm btn-warning mx-1 w-auto px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/truck-load-list">Cancel</router-link>
        </div>
        <div class="toolbar ml-auto">
    ...
    export default {
        ...
        computed: {
            ...
            recordList: function () {
                ...
                for (let opt of this.$store.state[this.axiosParams.module].recordList) {
                    let obj = {value: opt.id, text: opt.id.toString()};
                    options.push(obj);
                }
                if (this.searchTerm != '') {
        ...
        methods: {
            ...
            first: function () {
                ...
                eventBus.$emit(event.FIRST_LOADER_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_LOADER_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_LOADER_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_LOADER_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_LOADER, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_LOADER);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_LOADER, id);
            }
        }
    }
</script>
