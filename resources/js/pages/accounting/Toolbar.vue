<template>
    ...
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/accounting-list">Back To List</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_ACCOUNTING);
            },
            first: function(){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_ACCOUNTING_RECORD);
            },
            last: function(){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_ACCOUNTING_RECORD);
            },
            next: function(id){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_ACCOUNTING_RECORD, id);
            },
            prev: function(id){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_ACCOUNTING_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_ACCOUNTING, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_ACCOUNTING);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_ACCOUNTING, id);
            }
        }
    }
</script>
