<template>
    <div>
        <div class="non-print-elm" v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading" style="margin-bottom: 0rem !important;">Sandblasting Production Form</h5>
        ...
        <PDF :title="'printProductionSandblasting'">
            <div class="" slot="header">
                <div class="d-flex justify-content-center">
                    <h3>SANDBLASTING PRODUCTION FORM</h3>
                </div>
            </div>
            <div slot="body">
...
<script>
    ...
    export default {
        ...
        created() {
            eventBus.$on(event.SAVE_PRODUCTION_SANDBLASTING, this.save);
            eventBus.$on(event.DELETE_PRODUCTION_SANDBLASTING, this.deleteRecord);
            eventBus.$on(event.FIRST_PRODUCTION_SANDBLASTING_RECORD, this.first);
            eventBus.$on(event.LAST_PRODUCTION_SANDBLASTING_RECORD, this.last);
            eventBus.$on(event.NEXT_PRODUCTION_SANDBLASTING_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_PRODUCTION_SANDBLASTING_RECORD, this.prev);
            eventBus.$on(event.GET_PRODUCTION_SANDBLASTING, this.getRecord);
            eventBus.$on(event.ADD_PRODUCTION_SANDBLASTING, this.clearForm);
        },
        destroyed() {
            eventBus.$off(event.SAVE_PRODUCTION_SANDBLASTING);
            eventBus.$off(event.DELETE_PRODUCTION_SANDBLASTING);
            eventBus.$off(event.FIRST_PRODUCTION_SANDBLASTING_RECORD,);
            eventBus.$off(event.LAST_PRODUCTION_SANDBLASTING_RECORD);
            eventBus.$off(event.NEXT_PRODUCTION_SANDBLASTING_RECORD);
            eventBus.$off(event.PREVIOUS_PRODUCTION_SANDBLASTING_RECORD);
            eventBus.$off(event.GET_PRODUCTION_SANDBLASTING);
            eventBus.$off(event.ADD_PRODUCTION_SANDBLASTING);
        },
        beforeCreate() {
        ...
        methods: {
            ...
            getRecords() {
                ...
                fetchRecords({module: 'productionSandblasting/order'}, (response) => {
