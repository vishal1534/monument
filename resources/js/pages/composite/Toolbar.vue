<template>
    ...
    <div class="d-flex align-items-center w-100">
                    ...
                    <b-dropdown-group class="overflow-auto scroll input-md">
                        <div>
                            <div class="bg-toolbar text-center">
                                <label class="m-0">Composite</label>
                            </div>
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/composite-list">Back To List</router-link>
...
<script>
    ...
    export default {
        ...
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_COMPOSITE);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_COMPOSITE_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_COMPOSITE_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_COMPOSITE_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_COMPOSITE_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_COMPOSITE, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_COMPOSITE);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_COMPOSITE, id);
            },
        }
    }
</script>
