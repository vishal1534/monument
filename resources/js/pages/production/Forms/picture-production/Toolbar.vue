    ...
    export default {
        ...
        methods: {
            ...
            first: function () {
                ...
                eventBus.$emit(event.FIRST_PRODUCTION_PICTURE_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_PRODUCTION_PICTURE_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_PRODUCTION_PICTURE_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_PRODUCTION_PICTURE_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_PRODUCTION_PICTURE, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_PRODUCTION_PICTURE);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_PRODUCTION_PICTURE, id);
            }
        }
    }
</script>
