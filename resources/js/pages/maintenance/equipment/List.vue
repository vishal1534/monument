<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Equipment List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/equipment" v-if="isCreatePermitted">Add Equipment</router-link>
                </div>
                ...
                <div class="col-md-12">
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #head(date_purchased)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!singleDatePicker" v-model="equipmentDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="singleDatePicker" v-model="equipmentDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #cell(date_purchased)="data">
                                <span class="custom-table-body ml-1">{{ changeDateFormat(data.item.date_purchased) }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'equipment/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                        ...
                        <Modal :module="'Equipment'" :recordID="viewRecord" :title="'Equipment'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getEquipmentTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            filterRecords: async function (pageNo) {
                ...
                for (let key in this.queryFields) {
                    this.queryFields[key].filter(obj => {
                        ...
                        if (obj.operator == '...' && obj.column == 'date_purchased') {
                            isDateRangeSet = true
                            this.singleDatePicker = false
                            if (this.equipmentDate?.start && this.equipmentDate?.end) {
                                obj.value = this.equipmentDate.start + '...' + this.equipmentDate.end
                                isDateRangeSet = false
                            } else if (this.equipmentDate == undefined) {
                                obj.value = null
                                isDateRangeSet = false
                            }
                        } else if (obj.operator != '...' && obj.column == 'date_purchased') {
                            this.singleDatePicker = true
                            if (this.equipmentDate?.start && this.equipmentDate?.end) {
                                this.equipmentDate = null
                            }
                            obj.value = this.equipmentDate == undefined ? null : this.equipmentDate
                        }
                    })
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Equipment': {'columnTableName': 'equipment', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'equipment';
