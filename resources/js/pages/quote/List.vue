<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Quote List</h5>
        <div class="bg-toolbar my-2 p-1">

            <div class="d-flex justify-content-end align-items-center">

                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/quotes" v-if="isCreatePermitted">Add Quote</router-link>
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <div class="p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                 ...
                                 bordered
                                 class="my-table bootstrap-vue-table"
                                 hover
                                 id="data-table"
                                 responsive
                                 small
                            ...
                            <template #head(installation)="data">
                            ...
                            <template #head(call_customer)="data">
                                <div class="d-flex mx-1 custom-input-header">
                                    <div class="btn-group">
                                        <select @change="filterRecords(1)" class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).value">
                                            <option value="1">Yes</option>
                            ...
                            <template #head(callback_date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!callBackDatePicker" v-model="callBackDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="callBackDatePicker" v-model="callBackDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #head(date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!orderDatePicker" v-model="orderDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="orderDatePicker" v-model="orderDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #head(id)="data">
                                ...
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">Quote #</span>
                            </template>
                            ...
                            <template #cell(family_address)="data">
                                <p class="custom-table-body ml-1" style="word-break: break-all;"> {{ data.value }}</p>
                            </template>
                            ...
                            <template #cell(call_customer)="data">
                                <span class="custom-table-body ml-1"> {{ data.item.call_customer == 1 ? 'Yes' : 'No' }}</span>
                            </template>
                            <template #cell(date)="data">
                                <span class="custom-table-body ml-1"> {{ changeDateFormat(data.item.date) }}</span>
                            </template>
                            <template #cell(callback_date)="data">
                                <span class="custom-table-body ml-1"> {{ changeDateFormat(data.item.callback_date) }}</span>
                            </template>
                            <template #cell(installation)="data">
                                <span class="custom-table-body ml-1"> {{ data.item.installation == 1 ? 'Yes' : 'No' }}</span>
                            </template>
                            <template #cell(total_selling_price)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.total_selling_price ? data.item.total_selling_price.toFixed(2).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,") : '0.00' }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'quotes/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                        ...
                        <Modal :module="'Quote'" :recordID="viewRecord" :title="'Quote'"></Modal>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getQuoteTableData;
            },
            isBusy() {
            ...
            filterFields() {
                ...
                for (const [key, value] of Object.entries(this.$store.state[this.axiosParams.module].columnList)) {
                    ...
                    for (let val of value) {
                        let showColumn = displayCounter > 4 ? 'd-none' : '';// Only show first 4 columns by default
                        ...
                        if (val == 'id') { // to change label of Id to quote# for searching
                            label = 'Quote #';
                            showColumn = '';
                        }
                        if (val == 'sales_person') {
                            showColumn = 'd-none';
                        } else if (val == 'name_on_stone') {
                            label = "Last Name on Stone";
                        }
                        //console.log(customKey + ' : ' + val)
        ...
        methods: {
                ...
                for (let key in this.queryFields) {
                    this.queryFields[key].filter(obj => {
                        ...
                        if (obj.operator == '...' && obj.column == 'date') {
                            ...
                            this.orderDatePicker = true
                            if (this.orderDate?.start && this.orderDate?.end) {
                                this.orderDate = null
                            }
                            obj.value = this.orderDate == undefined ? null : this.orderDate
                        } else if (obj.operator == '...' && obj.column == 'callback_date') {
                            isDateRangeSet = true
                            this.callBackDatePicker = false
                            if (this.callBackDate?.start && this.callBackDate?.end) {
                                obj.value = this.callBackDate.start + '...' + this.callBackDate.end
                                isDateRangeSet = false
                            } else if (this.callBackDate == undefined) {
                                obj.value = null
                                isDateRangeSet = false
                            }
                        } else if (obj.operator != '...' && obj.column == 'callback_date') {
                            this.callBackDatePicker = true
                            if (this.callBackDate?.start && this.callBackDate?.end) {
                                this.callBackDate = null
                            }
                            obj.value = this.callBackDate == undefined ? null : this.callBackDate
                        }
                    })
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Cemetery': {'columnTableName': 'idInParentTable', 'foreignKey': 'cemetery_id', 'childClassName': '\\Cemetery'},
                    'Company': {'columnTableName': 'idInParentTable', 'foreignKey': 'company_id', 'childClassName': '\\Company'},
                    'Quote': {'columnTableName': 'quote', 'childClassName': ''}
                };
