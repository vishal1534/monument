<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/workorder-collaborator-list">Back To List</router-link>
        </div>
        <div class="toolbar ml-auto">
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            recordList: function () {
                //this.$store.getters.sortByAlphaNumeric({'module' : 'collaborator', 'fieldName': 'email'});
                let options = [];
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_COLLABORATOR);
            },
            first: function(){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_COLLABORATOR_RECORD);
            },
            last: function(){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_COLLABORATOR_RECORD);
            },
            next: function(id){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_COLLABORATOR_RECORD, id);
            },
            prev: function(id){
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_COLLABORATOR_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_COLLABORATOR, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_COLLABORATOR);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_COLLABORATOR, id);
            }
        }
    }
</script>
