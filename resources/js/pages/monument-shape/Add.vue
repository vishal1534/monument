<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Monument Shape</h5>
        ...
        <div id="monumentType" class="col-md-12">
            ...
            <form id="monument-form">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Monument Shape Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
                        <label class="font-weight-bold">Title:</label>
                        <b-form-input type="text" :disabled="hideInModal || hideinAuth" name="shape_title" class="input-xl" size="sm" v-model="formFields.shape_title"></b-form-input>
                    </div>
                </div>
                </div>
            </form>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'monumentShape'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Monument Shape`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_MONUMENT_SHAPE, this.storeMonumentShape);
            eventBus.$on(event.FIRST_MONUMENT_SHAPE_RECORD, this.first);
            eventBus.$on(event.LAST_MONUMENT_SHAPE_RECORD, this.last);
            eventBus.$on(event.NEXT_MONUMENT_SHAPE_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_MONUMENT_SHAPE_RECORD, this.prev);
            eventBus.$on(event.GET_MONUMENT_SHAPE, this.getMonumentShape);
            eventBus.$on(event.ADD_MONUMENT_SHAPE, this.clearForm);
            eventBus.$on(event.DELETE_MONUMENT_SHAPE, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_MONUMENT_SHAPE, this.storeMonumentShape);
            eventBus.$off(event.FIRST_MONUMENT_SHAPE_RECORD, this.first);
            eventBus.$off(event.LAST_MONUMENT_SHAPE_RECORD, this.last);
            eventBus.$off(event.NEXT_MONUMENT_SHAPE_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_MONUMENT_SHAPE_RECORD, this.prev);
            eventBus.$off(event.GET_MONUMENT_SHAPE, this.getMonumentShape);
            eventBus.$off(event.ADD_MONUMENT_SHAPE, this.clearForm);
            eventBus.$off(event.DELETE_MONUMENT_SHAPE, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getMonumentShape(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getMonumentShape: function(id){
            ...
            storeMonumentShape: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Monument Shape '+status, message : this.alertMessage, variant : 'success'});
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                            if (this.$store.state[this.axiosParams.module].recordList.length == 0){
                                ...
                                let params = {module: 'monument', detail: 'shape', id: this.axiosParams.id}
