<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Production List</h5>
                    ...
                    <div class="p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                ...
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!finalDatePicker" v-model="finalDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="finalDatePicker" v-model="finalDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #cell(final_date)="data">
                                <span class="custom-table-body ml-1">{{ changeDateFormat(data.item.final_date) }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'productions/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                               </div>
                            </template>
                        ...
                        <Modal :module="'Production'" :recordID="viewRecord" :title="'Production'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getProductionTableData(this.axiosParams);
            },
            perPageRecordList() {
            ...
            filterFields() {
                ...
                const labelMap = {
                    ...
                    original_invoice_number: 'Original Inv Number',
                };
                const showListColumn = ['date', 'name_on_stone', 'title', 'invoice_number'];
                const hideListColumn = ['original_invoice_number', 'date_promised', 'ship_date', 'closed_date'];
                ...
                const dropDownKeyMap = [
                    ...
                    {key: 'original_invoice_number', loc: 1, keyName: 'Inv/Order'},
                    {key: 'name_on_stone', loc: 2, keyName: 'Inv/Order'},
                    {key: 'contact', loc: 3, keyName: 'Inv/Order'},
                    {key: 'closed_date', loc: 4, keyName: 'Inv/Order'},
                    {key: 'ship_date', loc: 5, keyName: 'Inv/Order'},
                    {key: 'date_promised', loc: 6, keyName: 'Inv/Order'},
                    {key: 'invoice_total', loc: 7, keyName: 'Inv/Order'},
                    {key: 'date', loc: 8, keyName: 'Inv/Order'},
                    {key: 'po', loc: 9, keyName: 'Inv/Order'},
                    {key: 'sales_person', loc: 10, keyName: 'Inv/Order'},
                    {key: 'type_title', loc: 11, keyName: 'Inv/Order'},
                    {key: 'tax_rate', loc: 12, keyName: 'Inv/Order'},
                    {key: 'discount', loc: 13, keyName: 'Inv/Order'},
                ];
        ...
        methods: {
            ...
            filterRecords: async function (pageNo) {
                ...
                for (let key in this.queryFields) {
                    this.queryFields[key].filter(obj => {
                        ...
                        } else if (obj.operator == '...' && obj.column == 'final_date') {
                            isDateRangeSet = true
                            this.finalDatePicker = false
                            if (this.finalDate?.start && this.finalDate?.end) {
                                obj.value = this.finalDate.start + '...' + this.finalDate.end
                                isDateRangeSet = false
                            } else if (this.finalDate == undefined) {
                                obj.value = null
                                isDateRangeSet = false
                            }
                        } else if (obj.operator != '...' && obj.column == 'final_date') {
                            this.finalDatePicker = true
                            if (this.finalDate?.start && this.finalDate?.end) {
                                this.finalDate = null
                            }
                            obj.value = this.finalDate == undefined ? null : this.finalDate
                        }
                    })
            ...
            sortByAscDesc: function () {
                ...
                let tableNames = {
                    ...
                    'Production': {'columnTableName': 'production', 'childClassName': ''}
                };
                ...
                if (findParent != undefined && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'production';
