<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">User Role List</h5>
        <div class="bg-toolbar my-2 p-1">

            <div class="d-flex justify-content-end align-items-center">

                <!--<div class="d-flex m-auto">
                    <button class="btn custom-infobtn-list custom-infobtn-list-text" @click="formFields = {id:0}" v-b-modal.user-role-modal>Add New Role</button>
                </div>-->

                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/roles-manager">Add Role</router-link>
                </div>
                    ...
                    <div class="p-3">
                        ...
                        <div class="d-flex">
                            <b-dropdown
                                ...
                                <b-dropdown-form class="overflow-auto scroll" >
                        ...
                        <b-table id="data-table" class="my-table bootstrap-vue-table a" striped hover responsive bordered small
                            ...
                            <template #cell(action)="data" >
                                <div class="mx-1 custom-input-header">
                                    <button class="btn mx-1 py-1 custom-view-btn custom-view-text"  v-b-modal.view-modal @click="setViewRecord(data.item.id)">View</button>
                                    <router-link class="btn mx-1 py-1 custom-edit-btn custom-edit-text" :class="data.item.title == 'Customer'? 'd-none': data.item.title == 'Admin' ? 'd-none': '' " :to="'roles-manager/' + data.item.id" >Edit</router-link>
                                    <a class="btn mx-1 py-1 custom-delete-btn custom-delete-text" :class="data.item.title == 'Customer'? 'd-none': data.item.title == 'Admin' ? 'd-none': ''" href="javascript:void(0)" @click="deleteRecord(data.item.id)">Delete</a>
                                </div>
                            </template>

                        </b-table>

                        <Modal :module="'UserRole'" :title="'User Role'" :recordID="viewRecord"></Modal>
        ...
        <b-modal id="user-role-modal" title="Add Role">

            <template #modal-footer="{ ok, cancel, hide }">

                <b-button size="sm" variant="primary" @click="storeRole()">Add</b-button>
                <b-button size="sm" variant="secondary" @click="cancel()">Cancel</b-button>
            </template>

            <div class="custom-row d-block">
                <b-form-input type="text" size="sm" placeholder="Title" v-model="formFields.title"></b-form-input>
            </div>
...
<script>
    import {deleteRecord, saveRecord} from '../../helpers/axios';
    ...
    export default {
        ...
        computed:{
            ...
            tableData(){
                return this.$store.getters.getuserRoleTableData;
            },
            isBusy(){
