<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/purchase-order-list">Back To List</router-link>
            <button @click="printPdf" class="btn-info mx-1  custom-infobtn-toolbar custom-infobtn-text" type="button" v-if="id ? isUpdatePermitted : isCreatePermitted">
                Print Order
            </button>
        </div>
        <div class="toolbar ml-auto">
...
<script>
    ...
    export default {
        ...
        methods: {
            save: function () {
                eventBus.$emit(event.SAVE_PURCHASE_ORDER);
            },
            first: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.FIRST_PURCHASE_ORDER_RECORD);
            },
            last: function () {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_PURCHASE_ORDER_RECORD);
            },
            next: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_PURCHASE_ORDER_RECORD, id);
            },
            prev: function (id) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_PURCHASE_ORDER_RECORD, id);
            },
            getRecord: function (value) {
                this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_PURCHASE_ORDER, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_PURCHASE_ORDER);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_PURCHASE_ORDER, id);
            },
            printPdf() {
