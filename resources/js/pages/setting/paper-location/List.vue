<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Paper Location List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/paper-location">Add Paper Location</router-link>
                </div>
                <div class="d-flex">
                </div>
            ...
            <div class="row m-0">
                <div class="col-md-12">
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'paper-location/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                        ...
                        <Modal :module="'PaperLocation'" :recordID="viewRecord" :title="'Paper Location'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                sortByColumn: 'location_title',
                sortByDesc: false,
                columnTableName: 'paperLocation',
                foreignKey: '',
                childClassName: '',
                childTableName: '',
                sortDirection: 'asc',
        ...
        metaInfo() {
            return {
                title: `Paper Location Info. List`,
            }
        },
        computed: {
            perPageRecordList() {
            ...
            tableData() {
                return this.$store.getters.getPaperLocationTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'PaperLocation': {'columnTableName': 'paperLocation', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'paperLocation';
                    this.sortByColumn = 'location_title'
                }
