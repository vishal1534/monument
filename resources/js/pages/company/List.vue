<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Company List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <div class="d-flex m-auto">
                    <router-link class="btn custom-infobtn-list custom-infobtn-list-text" to="/company" v-if="isCreatePermitted">Add Customer</router-link>
                </div>
                <div class="d-flex">
                </div>
                    ...
                    <div class="quote-body p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                            ...
                            <template #cell(title)="data">
                                <span class="custom-table-body ml-1"> {{ data.item.type ? data.item.type.title : '' }}</span>
                            </template>
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'company/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                        ...
                        <Modal :module="'Company'" :recordID="viewRecord" :title="'Company'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getCompanyTableData;
            },
            perPageRecordList() {
            ...
            filterFields() {
                ...
                for (const [key, value] of Object.entries(this.$store.state[this.axiosParams.module].columnList)) {
                    ...
                    for (let val of value) {
                        ...
                        if (val == 'address' || val == 'account_number') {
                            showColumn = 'd-none';
                        }
                        ...
                        if (val == 'phone_2' || val == 'email_2' || val == 'contact_2') { // to change label of phone,Email and contact 2 to #2 for searching
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Type': {'columnTableName': 'idInParentTable', 'foreignKey': 'company_info_type_id', 'childClassName': '\\CompanyInfoType'},
                    'Company': {'columnTableName': 'company', 'childClassName': ''}
                };
