<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Installation List</h5>
                ...
                <div class="col-md-12">
                    <div class="p-3">
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                 ...
                                 bordered class="my-table bootstrap-vue-table" hover
                            ...
                            <template #head(final_date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        ...
                                        <b-form-datepicker :date-format-options="{ year: 'numeric', month: 'numeric', day: 'numeric' }" @input="filterRecords(1)" class="form-control-sm rounded-0 border" close-button locale="en" placeholder=""
                                                           reset-button
                                                           size="sm"
                                                           today-button
                            ...
                            <template #head(install_date)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        <select @change="filterRecords(1)" class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).operator">
                                            <option title="Equals To" v-b-tooltip.hover value="like">=</option>
                                        ...
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" range v-if="!installDatePicker" v-model="installedDate"/>
                                        <VueDatePicker :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @input="filterRecords(1)" class="form-control-sm rounded-0 border"
                                                       clearable
                                                       format="MM/DD/YY" no-calendar-icon no-header placeholder="MM/DD/YY" v-if="installDatePicker" v-model="installedDate"/>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #head(date_promised)="data">
                                <div class="d-flex mx-1 custom-date-header">
                                    <div class="btn-group">
                                        <select class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).operator">
                                            <option title="Equals To" v-b-tooltip.hover value="like">=</option>
                                            <option title="Empty" v-b-tooltip.hover value="IS">=''</option>
                                            <option title="Not Equals to" v-b-tooltip.hover value="not like"><></option>
                                            <option title="Greater Than" v-b-tooltip.hover value=">">></option>
                            ...
                            <template #head(customer_called_after_installation)="data">
                                <div class="d-flex mx-1" style="width: 17rem; margin-bottom: 0.5rem !important;">
                                    <div class="btn-group">
                                        <select @change="filterRecords(1)" class="form-control-sm rounded-0 border bg-light" v-model="queryFields[data.field.parent].find(item => item.column == data.column).operator">
                                        ...
                                        <b-form-input :disabled="queryFields[data.field.parent].find(item => item.column == data.column).operator == 'IS' ? true : false " @change="filterRecords(1)" class="form-control-sm rounded-0 border" placeholder="Search here..."
                                                      size="sm"
                                                      v-model="queryFields[data.field.parent].find(item => item.column == data.column).value"></b-form-input>
                                    </div>
                                </div>
                                <hr class="border">
                                <span class="ml-2 mr-1 custom-table-heading">{{ data.label }}</span>
                            ...
                            <template #cell(date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.date ? changeDateFormat(data.item.order.date) : '' }}</span>
                            </template>
                            <template #cell(closed_date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.invoice?.closed_date ? changeDateFormat(data.item.order.invoice.closed_date) : '' }}</span>
                            </template>
                            <template #cell(date_promised)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.invoice?.date_promised ? changeDateFormat(data.item.order.invoice.date_promised) : '' }}</span>
                            </template>
                            <template #cell(ship_date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.invoice?.ship_date ? changeDateFormat(data.item.order.invoice.ship_date) : '' }}</span>
                            </template>
                            <template #cell(install_date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.installation?.install_date ? changeDateFormat(data.item.order.installation.install_date) : '' }}</span>
                            </template>
                            <template #cell(promised_date)="data">
                                <span class="custom-table-body ml-1">{{ data.item.order?.installation?.date_promised ? changeDateFormat(data.item.order.installation.date_promised) : '' }}</span>
                            </template>
                            <template #cell(tax_amount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order ? convertDecimal(Number(data.item.order.tax_amount)) : '0.00' }}</span>
                            </template>
                            <template #cell(tax_rate)="data">
                                <span class="custom-table-body ml-1">% {{ data.item.order ? convertDecimal(Number(data.item.order.tax_rate)) : '0.00' }}</span>
                            </template>
                            <template #cell(sub_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order ? convertDecimal(Number(data.item.order.sub_total)) : '0.00' }}</span>
                            </template>
                            <template #cell(invoice_total)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order ? convertDecimal(Number(data.item.order.invoice.invoice_total)) : '0.00' }}</span>
                            </template>
                            <template #cell(discount)="data">
                                <span class="custom-table-body ml-1">$ {{ data.item.order ? convertDecimal(Number(data.item.order.discount)) : '0.00' }}</span>
                            </template>
                            <template #cell(status)="data">
                                <span class="custom-table-body ml-1"> {{ data.item.order?.order_status_id ? customizeLabel(data.item.order.order_status.title) : '' }}</span>
                            </template>
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    ...
                                    <router-link :to="'installation/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'Installation'" :recordID="viewRecord" :title="'Installation'"></Modal>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            tableData() {
                return this.$store.getters.getInstallationTableData;
            },
            perPageRecordList() {
        ...
        methods: {
            ...
            filterRecords: async function (pageNo) {
                ...
                for (let key in this.queryFields) {
                    this.queryFields[key].filter(obj => {
                        ...
                        } else if (obj.operator == '...' && obj.column == 'install_date') {
                            isDateRangeSet = true
                            this.installDatePicker = false
                            if (this.installedDate?.start && this.installedDate?.end) {
                                obj.value = this.installedDate.start + '...' + this.installedDate.end
                                isDateRangeSet = false
                            } else if (this.installedDate == undefined) {
                                obj.value = null
                                isDateRangeSet = false
                            }
                        } else if (obj.operator != '...' && obj.column == 'install_date') {
                            this.installDatePicker = true
                            if (this.installedDate?.start && this.installedDate?.end) {
                                this.installedDate = null
                            }
                            obj.value = this.installedDate == undefined ? null : this.installedDate
                        }
                    })
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Invoice': {'columnTableName': 'idInChildTable', 'tableName': 'invoices', 'childClassName': '\\Invoice'},
                    'Family': {'columnTableName': 'idInChildTable', 'tableName': 'families', 'childClassName': '\\Family'},
                    ...
                    'Installation': {'columnTableName': 'installation', 'childClassName': ''}
                };

                let findParent = this.tableFields.find(obj => obj.key == this.sortByColumn)
                if (findParent != undefined && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'installation';
            ...
            toggleColumn: function (column) {
                this.tableFields.filter(obj => {
                    if ((obj.key == column.key) && (obj.parent == column.parent)) {
