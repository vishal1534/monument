<template>
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/maintenance-work-order-list">Back To List</router-link>
    ...
    export default {
        ...
        methods: {
            ...
            first: function () {
                ...
                eventBus.$emit(event.FIRST_WORK_ORDER_RECORD);
            },
            last: function () {
                 this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.LAST_WORK_ORDER_RECORD);
            },
            next: function (id) {
                 this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.NEXT_WORK_ORDER_RECORD, id);
            },
            prev: function (id) {
                 this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.PREVIOUS_WORK_ORDER_RECORD, id);
            },
            getRecord: function (value) {
                 this.$store.state[this.axiosParams.module].busy = true
                eventBus.$emit(event.GET_WORK_ORDER, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_WORK_ORDER);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_WORK_ORDER, id);
            },
            printPDF() {
