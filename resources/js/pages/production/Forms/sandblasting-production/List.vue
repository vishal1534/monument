<template>
    <div>
        <h5 class="mx-2 my-1 custom-view-heading">Sandblasting Production Forms List</h5>
        <div class="bg-toolbar my-2 p-1">
            <div class="d-flex justify-content-end align-items-center">
                <router-link class="btn custom-infobtn-list custom-infobtn-list-text ml-4" to="/sandblasting-production" v-if="isCreatePermitted">Add New</router-link>
                <div class="d-flex m-auto">
                    <button class="btn-info mx-1 custom-infobtn-toolbar custom-infobtn-text" type="button" v-if="false">Print List ?</button>
                </div>
                <div class="d-flex">
                </div>
        ...
        <div>
            <div class="row m-0">
                <div class="col-md-12">
                    <div class="installation-body p-3">
                        <div class="d-flex">
                            <b-dropdown
                                ...
                                <b-dropdown-form class="overflow-auto scroll" style="height: 28vh;">
                                    <template v-for="(objects, key) of filterFields">
                                        <h6>Production Sandblasting</h6>
                        ...
                        <b-table :busy="isBusy" :current-page="currentPage" :fields="tableFields" :items="tableData" :no-local-sorting="true" :per-page="pagePerPage"
                                 ...
                                 bordered class="my-table bootstrap-vue-table"
                            ...
                            <template #cell(action)="data">
                                <div class="mx-1 custom-input-header">
                                    <b-button @click="setViewRecord(data.item.id)" class="btn mx-1 py-1 custom-view-btn custom-view-text" v-b-modal.view-modal v-if="false">View</b-button>
                                    <router-link :to="'/sandblasting-production/' + data.item.id" class="btn mx-1 py-1 custom-edit-btn custom-edit-text" v-if="isUpdatePermitted">Edit</router-link>
                                    <a @click="deleteRecord(data.item.id)" class="btn mx-1 py-1 custom-delete-btn custom-delete-text" href="javascript:void(0)" v-if="false">Delete</a>
                                </div>
                            </template>
                        </b-table>
                        <Modal :module="'ProductionSandBlasting'" :recordID="viewRecord" :title="'Sandblasting Production Form'"></Modal>
                    </div>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                sortByColumn: 'id',
                sortByDesc: false,
                columnTableName: 'productionSandblasting',
                foreignKey: '',
                childClassName: '',
                childTableName: '',
                sortDirection: 'desc',
        ...
        metaInfo() {
            return {
                title: `Sandblasting Production Forms List`,
            }
        },
        computed: {
            perPageRecordList() {
            ...
            tableData() {
                return this.$store.getters.getProductionSandblastingTableData;
            },
            isBusy() {
        ...
        methods: {
            ...
            sortByAscDesc: function () {
                this.$store.state[this.axiosParams.module].busy = true
                let tableNames = {
                    'Production Sandblast': {'columnTableName': 'productionSandblasting', 'childClassName': ''}
                };
                ...
                if (this.sortByColumn != 'id' && tableNames[findParent.parent]) {
                    ...
                    this.columnTableName = 'productionSandblasting';
