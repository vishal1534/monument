<template>
    <div>
        <div class="non-print-elm" v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading" style="margin-bottom: 0rem !important;">Laser Production Form</h5>
        ...
        <PDF :title="'printProductionLaser'">
            <div class="" slot="header">
                <div class="d-flex justify-content-center">
                    <h3>LASER PRODUCTION FORM</h3>
                </div>
            </div>
            <div slot="body">
...
<script>
    ...
    export default {
        ...
        created() {
            eventBus.$on(event.SAVE_PRODUCTION_LASER, this.save);
            eventBus.$on(event.FIRST_PRODUCTION_LASER_RECORD, this.first);
            eventBus.$on(event.LAST_PRODUCTION_LASER_RECORD, this.last);
            eventBus.$on(event.NEXT_PRODUCTION_LASER_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_PRODUCTION_LASER_RECORD, this.prev);
            eventBus.$on(event.GET_PRODUCTION_LASER, this.getRecord);
            eventBus.$on(event.DELETE_PRODUCTION_LASER, this.deleteRecord);
            eventBus.$on(event.ADD_PRODUCTION_LASER, this.clearForm);
        },
        destroyed() {
            eventBus.$off(event.SAVE_PRODUCTION_LASER);
            eventBus.$off(event.FIRST_PRODUCTION_LASER_RECORD,);
            eventBus.$off(event.LAST_PRODUCTION_LASER_RECORD);
            eventBus.$off(event.NEXT_PRODUCTION_LASER_RECORD);
            eventBus.$off(event.PREVIOUS_PRODUCTION_LASER_RECORD);
            eventBus.$off(event.GET_PRODUCTION_LASER);
            eventBus.$off(event.DELETE_PRODUCTION_LASER);
            eventBus.$off(event.ADD_PRODUCTION_LASER);
        },
        beforeCreate() {
        ...
        methods: {
            ...
            getRecords() {
                ...
                fetchRecords({module: 'productionLaser/order'}, (response) => {
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data) {
                            ...
                            if (this.$store.state[this.axiosParams.module].firstRecord)
                                ...
                                this.formFields = {id: 0, laser_list: []};
                            if (this.$store.state[this.axiosParams.module].recordList.length == 0)
                                this.clearForm();
                        } else {
