<template>
    ...
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar">
            ...
            <button type="button" class="btn btn-sm custom-label-toolbar" :disabled="title == 'Customer' ? true : false  " @click="deleteRecord(id)">Delete Record</button>
        </div>

        <div class="toolbar ml-auto">
            <button type="button" class="btn-info mx-1  custom-infobtn-toolbar custom-infobtn-text" @click="save" :disabled="title == 'Customer' ? true  :saveDisabled">
                {{ id ? 'Update' : 'Save' }}<span v-if="saveDisabled" class="spinner-grow spinner-grow-sm ml-1"></span>
            </button>
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/roles-list">Cancel</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        computed:{
            recordList: function () {
                let options = [];
                for (let opt of this.$store.state[this.axiosRoleParams.module].recordList){
            ...
            totalRecords: function () {
                return this.$store.state[this.axiosRoleParams.module].recordList.length;
            },
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_USER_ROLE);
            },
            first: function(){
                eventBus.$emit(event.FIRST_USER_ROLE_RECORD);
            },
            last: function(){
                eventBus.$emit(event.LAST_USER_ROLE_RECORD);
            },
            next: function(id){
                eventBus.$emit(event.NEXT_USER_ROLE_RECORD, id);
            },
            prev: function(id){
                eventBus.$emit(event.PREVIOUS_USER_ROLE_RECORD, id);
            },
            getRecord: function (value) {
                eventBus.$emit(event.GET_USER_ROLE, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_USER_ROLE);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_USER_ROLE, id);
            }
        }
    }
</script>
