...
use App\Models\WorkOrderCollaboratorType;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class WorkOrderCollaboratorsController extends Controller
{
    ...
    {
        $records = new WorkOrderCollaborator();
    ...
    {
        $records = WorkOrderCollaborator::select('id', 'email')
            ->orderBy('email', 'asc')
            ->get();
        return response()->json($records);
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $recordInstance = WorkOrderCollaborator::updateOrCreate(['id' => $data['id']], $data);
    ...
    public function show(WorkOrderCollaborator $collaborator)
    {
        $collaborator->load($this->relations);
        return response()->json($collaborator);
    }

    public function destroy(WorkOrderCollaborator $collaborator)
    {
    ...
    {
        $collaborators = new WorkOrderCollaborator();
        $collaboratorsType = new WorkOrderCollaboratorType();
    ...
    {
        $record = new WorkOrderCollaborator();
        $request = $request->all();
    ...
    {
        $collaboratorType = new WorkOrderCollaboratorType();
        return response()->json($collaboratorType->select('id', 'title')->orderBy('title', 'asc')->get());
    }
    ...
    public function saveCollaboratorType(Request $request)
    {
        $data = $request->all();
        // $data = json_decode($request->data, true);
        $collaboratorType = WorkOrderCollaboratorType::updateOrCreate(['id' => $data['id']], $data);
        $collaboratorType->save();
        return response()->json($collaboratorType);
    }

    public function deleteCollaboratorType(WorkOrderCollaboratorType $collaboratorType)
    {
        return $collaboratorType->delete();
    }

    public function getNextPrevious(WorkOrderCollaborator $collaborator)
    {
        return $this->nextPreviousRecord(new WorkOrderCollaborator, $this->relations, $collaborator->id);
    }
