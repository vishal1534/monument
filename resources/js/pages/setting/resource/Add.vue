<template>
    <div>
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} User Resources Information</h5>
                    ...
                    <form >
                        ...
                        <div class="div-shadow bg-white rounded p-2 mb-2">
                            ...
                            <div class="company-row">
                                <div>
                                    <h6 class="my-auto ml-2 custom-form-heading">User Resources Information</h6>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Resources`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_RESOURCE, this.storeResource);
            eventBus.$on(event.FIRST_RESOURCE_RECORD, this.first);
            eventBus.$on(event.LAST_RESOURCE_RECORD, this.last);
            eventBus.$on(event.NEXT_RESOURCE_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_RESOURCE_RECORD, this.prev);
            eventBus.$on(event.GET_RESOURCE, this.getCurrentResource);
            eventBus.$on(event.ADD_RESOURCE, this.clearForm);
            eventBus.$on(event.DELETE_RESOURCE, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_RESOURCE, this.storeResource);
            eventBus.$off(event.FIRST_RESOURCE_RECORD, this.first);
            eventBus.$off(event.LAST_RESOURCE_RECORD, this.last);
            eventBus.$off(event.NEXT_RESOURCE_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_RESOURCE_RECORD, this.prev);
            eventBus.$off(event.GET_RESOURCE, this.getCurrentResource);
            eventBus.$off(event.ADD_RESOURCE, this.clearForm);
            eventBus.$off(event.DELETE_RESOURCE, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getCurrentResource(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getCurrentResource: function(id){
