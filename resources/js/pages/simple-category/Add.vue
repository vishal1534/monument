<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Simple Product Category</h5>
        ...
        <div id="monumentType" class="col-md-12">
            ...
            <form id="monument-form">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Simple Product Category Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'simpleCategory'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Simple Category`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_SIMPLE_CATEGORY, this.storeSimpleCategory);
            eventBus.$on(event.FIRST_SIMPLE_CATEGORY_RECORD, this.first);
            eventBus.$on(event.LAST_SIMPLE_CATEGORY_RECORD, this.last);
            eventBus.$on(event.NEXT_SIMPLE_CATEGORY_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_SIMPLE_CATEGORY_RECORD, this.prev);
            eventBus.$on(event.GET_SIMPLE_CATEGORY, this.getSimpleCategory);
            eventBus.$on(event.ADD_SIMPLE_CATEGORY, this.clearForm);
            eventBus.$on(event.DELETE_SIMPLE_CATEGORY, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_SIMPLE_CATEGORY, this.storeSimpleCategory);
            eventBus.$off(event.FIRST_SIMPLE_CATEGORY_RECORD, this.first);
            eventBus.$off(event.LAST_SIMPLE_CATEGORY_RECORD, this.last);
            eventBus.$off(event.NEXT_SIMPLE_CATEGORY_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_SIMPLE_CATEGORY_RECORD, this.prev);
            eventBus.$off(event.GET_SIMPLE_CATEGORY, this.getSimpleCategory);
            eventBus.$off(event.ADD_SIMPLE_CATEGORY, this.clearForm);
            eventBus.$off(event.DELETE_SIMPLE_CATEGORY, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getSimpleCategory(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getSimpleCategory: function(id){
            ...
            storeSimpleCategory: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Simple Category '+status, message : this.alertMessage, variant : 'success'});
                ...
                if (confirm("Do you really want to delete this record?")){
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                                ...
                                let params = {module: 'simple', detail: 'category', id: this.axiosParams.id}
