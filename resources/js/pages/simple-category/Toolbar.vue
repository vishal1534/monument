<template>
    ...
    <div class="d-flex align-items-center w-100">
        ...
        <div class="toolbar ml-auto">
            ...
            <router-link class="btn btn-sm btn-warning mx-1 px-3 custom-dangerbtn-toolbar custom-dangerbtn-text" to="/simple-category-list">Back To List</router-link>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            save:function(){
                eventBus.$emit(event.SAVE_SIMPLE_CATEGORY);
            },
            first: function(){
                eventBus.$emit(event.FIRST_SIMPLE_CATEGORY_RECORD);
            },
            last: function(){
                eventBus.$emit(event.LAST_SIMPLE_CATEGORY_RECORD);
            },
            next: function(id){
                eventBus.$emit(event.NEXT_SIMPLE_CATEGORY_RECORD, id);
            },
            prev: function(id){
                eventBus.$emit(event.PREVIOUS_SIMPLE_CATEGORY_RECORD, id);
            },
            getRecord: function (value) {
                eventBus.$emit(event.GET_SIMPLE_CATEGORY, value);
            },
            addRecord: function () {
                eventBus.$emit(event.ADD_SIMPLE_CATEGORY);
            },
            deleteRecord: function (id) {
                eventBus.$emit(event.DELETE_SIMPLE_CATEGORY, id);
            }
        }
    }
</script>
