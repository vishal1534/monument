<template>
    <div :class="hideInModal?' custom-modal-margin':''">
        <div v-if="!hideInModal">
            <h5 class="mx-2 my-1 custom-view-heading"> {{ formFields.id ? 'Edit' : 'Add'}} Monument Color</h5>
            ...
            <form id="monument-form">
               ...
               <div class="pt-1">
                ...
                <div class="product-row">
                    <div>
                        <h6 class="my-auto ml-2 custom-form-heading">Monument Color Information</h6>
                    </div>
                </div>
                <div class="product-row">
                    <div>
                        <label class="font-weight-bold">Title:</label>
                        <b-form-input type="text" :disabled="hideInModal || hideinAuth" name="color_title" class="input-xl" size="sm" v-model="formFields.color_title"></b-form-input>
                    </div>
                </div>
                </div>
    ...
    export default {
        ...
        data() {
            return  {
                ...
                axiosParams: {module: 'monumentColor'},
                alertMessage:'',
                hideAlert:true,
        ...
        metaInfo() {
            return {
                title: `${this.formFields.id ? 'Edit' : 'Add'} Monument Color`,
            }
        },
        created() {
            eventBus.$on(event.SAVE_MONUMENT_COLOR, this.storeMonumentColor);
            eventBus.$on(event.FIRST_MONUMENT_COLOR_RECORD, this.first);
            eventBus.$on(event.LAST_MONUMENT_COLOR_RECORD, this.last);
            eventBus.$on(event.NEXT_MONUMENT_COLOR_RECORD, this.next);
            eventBus.$on(event.PREVIOUS_MONUMENT_COLOR_RECORD, this.prev);
            eventBus.$on(event.GET_MONUMENT_COLOR, this.getMonumentColor);
            eventBus.$on(event.ADD_MONUMENT_COLOR, this.clearForm);
            eventBus.$on(event.DELETE_MONUMENT_COLOR, this.deleteRecord);
        },
        destroyed() {
            eventBus.$off(event.SAVE_MONUMENT_COLOR, this.storeMonumentColor);
            eventBus.$off(event.FIRST_MONUMENT_COLOR_RECORD, this.first);
            eventBus.$off(event.LAST_MONUMENT_COLOR_RECORD, this.last);
            eventBus.$off(event.NEXT_MONUMENT_COLOR_RECORD, this.next);
            eventBus.$off(event.PREVIOUS_MONUMENT_COLOR_RECORD, this.prev);
            eventBus.$off(event.GET_MONUMENT_COLOR, this.getMonumentColor);
            eventBus.$off(event.ADD_MONUMENT_COLOR, this.clearForm);
            eventBus.$off(event.DELETE_MONUMENT_COLOR, this.deleteRecord);
        },
        components:{
            Toolbar
        },
        ...
        mounted() {
            ...
            this.getMonumentColor(this.formFields.id);
            this.getColumns();
            this.getRecords();
        },
        methods:{
            /** CRUD */
            getMonumentColor: function(id){
            ...
            storeMonumentColor: function(){
                ...
                saveRecord(this.axiosParams, formData, (response) => {
                    if (response.data.errorInfo){
                        ...
                        this.makeToast({title : 'Monument Color '+status, message : this.alertMessage, variant : 'success'});
                    ...
                    deleteRecord(this.axiosParams, (response) => {
                        if (response.data){
                            if (this.$store.state[this.axiosParams.module].recordList.length == 0){
                                ...
                                let params = {module: 'monument', detail: 'color', id: this.axiosParams.id}
